...
void get_datetime_string(time_t * raw_time, char *buffer, int buffer_length,
	...
	if (type == LONG_DATE_TIME){
		if (date_format==DATE_FORMAT_ISO8601 || date_format==DATE_FORMAT_STRICT_ISO8601)
            snprintf(buffer,buffer_length,"%s %d年%s月%d日%c%02d时:%02d分:%02d秒 %s",
                     weekdays[tm_ptr->tm_wday],year,months[tm_ptr->tm_mon],day,
                     (date_format==DATE_FORMAT_STRICT_ISO8601)?'T':' ',hour,minute,second,tzone);
		else
	...
	/* short date/time */
		...
		if (date_format == DATE_FORMAT_EURO)
			...
			snprintf(buffer, buffer_length,
                     "%04d年%02d月%02d日%c%02d时:%02d分:%02d秒", year, month,
                     day,
                     (date_format ==
	...
	/* short date */
		...
		if (date_format == DATE_FORMAT_EURO)
			snprintf(buffer, buffer_length, "%02d-%02d-%04d", day,
                 ...
                 || date_format == DATE_FORMAT_STRICT_ISO8601)
			snprintf(buffer, buffer_length, "%04d年%02d月%02d日", year,
                     month, day);
		else
	...
	/* expiration date/time for HTTP headers */
        ...
        if (date_format==DATE_FORMAT_ISO8601 || date_format==DATE_FORMAT_STRICT_ISO8601)
            snprintf(buffer, buffer_length,
                     "%s %d年%s月%02d日%c%02d时:%02d分:%02d秒 GMT",
                     weekdays[tm_ptr->tm_wday], year, months[tm_ptr->tm_mon], 
                     day, (date_format==DATE_FORMAT_STRICT_ISO8601)?'T':' ', hour, minute, second);
        else snprintf(buffer, buffer_length,
	...
	/* short time */
        ...
        if (date_format==DATE_FORMAT_ISO8601 || date_format==DATE_FORMAT_STRICT_ISO8601)
            snprintf(buffer, buffer_length, "%02d时:%02d分:%02d秒", hour, minute,
                     second);
		else
