	...
	if (temp_downtime->is_in_effect == TRUE) {
		...
		if (temp_downtime->type == HOST_DOWNTIME) {
			...
			if (hst->scheduled_downtime_depth == 0) {
				...
				logit(NSLOG_INFO_MESSAGE, FALSE, "主机宕机警告: %s;取消; 安排主机宕机已取消.\n", hst->name);
			...
			if (svc->scheduled_downtime_depth == 0) {
				...
				logit(NSLOG_INFO_MESSAGE, FALSE, "服务宕机警告: %s;%s;取消; 安排服务宕机已取消.\n", svc->host_name, svc->description);
...
int register_downtime(int type, unsigned long downtime_id) {
	...
	if (temp_downtime->type == HOST_DOWNTIME) {
		log_debug_info(DEBUGL_DOWNTIME, 0, " 类型:        主机宕机\n");
		log_debug_info(DEBUGL_DOWNTIME, 0, " 主机:        %s\n", hst->name);
	} else {
		log_debug_info(DEBUGL_DOWNTIME, 0, " 类型:        服务宕机\n");
		log_debug_info(DEBUGL_DOWNTIME, 0, " 主机:        %s\n", svc->host_name);
		log_debug_info(DEBUGL_DOWNTIME, 0, " 服务:     %s\n", svc->description);
	}
	log_debug_info(DEBUGL_DOWNTIME, 0, " 固定/可变:  %s\n", (temp_downtime->fixed == TRUE) ? "Fixed" : "Flexible");
	log_debug_info(DEBUGL_DOWNTIME, 0, " 开始:       %s\n", start_time_string);
	log_debug_info(DEBUGL_DOWNTIME, 0, " 结束:         %s\n", end_time_string);
	log_debug_info(DEBUGL_DOWNTIME, 0, " 持续时间:    %2d时:%2d分:%2d秒\n", hours, minutes, seconds);
	log_debug_info(DEBUGL_DOWNTIME, 0, " 宕机ID: %lu\n", temp_downtime->downtime_id);
	log_debug_info(DEBUGL_DOWNTIME, 0, " 触发ID:  %lu\n", temp_downtime->triggered_by);


	/* add a non-persistent comment to the host or service regarding the scheduled outage */
	if (temp_downtime->type == SERVICE_DOWNTIME)
		add_new_comment(SERVICE_COMMENT, DOWNTIME_COMMENT, svc->host_name, svc->description, time(NULL), "(Icinga进程)", temp_buffer, 0, COMMENTSOURCE_INTERNAL, FALSE, (time_t)0, &(temp_downtime->comment_id));
	else
		add_new_comment(HOST_COMMENT, DOWNTIME_COMMENT, hst->name, NULL, time(NULL), "(Icinga进程)", temp_buffer, 0, COMMENTSOURCE_INTERNAL, FALSE, (time_t)0, &(temp_downtime->comment_id));
...
int handle_scheduled_downtime(scheduled_downtime *temp_downtime) {
	...
	if (temp_downtime->is_in_effect == TRUE && ( /* downtime needs to be in effect and ... */
		...
		if (temp_downtime->type == HOST_DOWNTIME && hst->scheduled_downtime_depth == 0) {
			...
			log_debug_info(DEBUGL_DOWNTIME, 0, "主机'%s'已从安排宕机时段退出(id=%lu).\n", hst->name, temp_downtime->downtime_id);

			/* log a notice - this one is parsed by the history CGI */
			logit(NSLOG_INFO_MESSAGE, FALSE, "主机宕机警告: %s;停止; 主机已从安排宕机时段退出", hst->name);
			...
			log_debug_info(DEBUGL_DOWNTIME, 0, "主机'%s'上的'%s'服务已从安排宕机时段退出 (id=%lu).\n", svc->host_name, svc->description, temp_downtime->downtime_id);

			/* log a notice - this one is parsed by the history CGI */
			logit(NSLOG_INFO_MESSAGE, FALSE, "服务宕机警告: %s;%s;停止; 服务已从安排宕机时段退出", svc->host_name, svc->description);
