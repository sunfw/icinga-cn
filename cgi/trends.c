...
int main(int argc, char **argv) {
	...
	document_header(CGI_ID, TRUE, "趋势");
	...
	if (content_type == HTML_CONTENT && display_header == TRUE) {
		...
		if (display_type == DISPLAY_HOST_TRENDS)
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "主机状态趋势");
		else if (display_type == DISPLAY_SERVICE_TRENDS)
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "服务状态趋势");
		else
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "主机和服务状态趋势");
		...
		if (display_type != DISPLAY_NO_TRENDS && input_type == GET_INPUT_NONE) {
			...
			if (display_type == DISPLAY_HOST_TRENDS)
				printf("主机 '%s'", (temp_host->display_name != NULL) ? temp_host->display_name : temp_host->name);
			else if (display_type == DISPLAY_SERVICE_TRENDS)
				printf("主机'%s'上的服务'%s'", (temp_host->display_name != NULL) ? temp_host->display_name : temp_host->name, (temp_service->display_name != NULL) ? temp_service->display_name : temp_service->description);
			printf("</DIV>\n");
			...
			printf("<div align=center class='reportRange'>%s到%s</div>\n", start_timestring, end_timestring);
		...
		if (display_type != DISPLAY_NO_TRENDS && input_type == GET_INPUT_NONE) {
			...
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>首先假定%s的状态:</td><td CLASS='optBoxItem' valign=top align=left>返回存档:</td></tr>\n", (display_type == DISPLAY_HOST_TRENDS) ? "主机" : "服务");
			...
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>报告时间段:</td><td CLASS='optBoxItem' valign=top align=left>缩放倍数:</td></tr>\n");
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>\n");
			...
			if (display_type == DISPLAY_HOST_TRENDS)
				printf("<option value=nextproblem %s>下一个主机故障\n", (timeperiod_type == TIMEPERIOD_NEXTPROBLEM) ? "SELECTED" : "");
			else
				printf("<option value=nextproblem %s>下一个服务故障\n", (timeperiod_type == TIMEPERIOD_NEXTPROBLEM) ? "SELECTED" : "");
			printf("</select>\n");
			printf("</td><td CLASS='optBoxItem' valign=top align=left>\n");
	...
	if (is_authorized == FALSE) {
		...
		if (content_type == HTML_CONTENT) {
			if (display_type == DISPLAY_HOST_TRENDS)
				print_generic_error_message("很显然您无权访问该特定主机的信息...", NULL, 0);
			else
				print_generic_error_message("很显然您无权访问该特定服务的信息...", NULL, 0);
		}
	...
	if (timeperiod_type == TIMEPERIOD_NEXTPROBLEM && problem_found == FALSE) {
		print_generic_error_message("在显示结尾和记录结尾之间，没有发现故障.", NULL, 0);
	...
	if (display_type != DISPLAY_NO_TRENDS && content_type == IMAGE_CONTENT) {
		...
		if (small_image == FALSE) {
			...
			if( (strlen(font_file) > 5) && (access(font_file, F_OK) == 0) ){
				if(display_type==DISPLAY_HOST_TRENDS)
					snprintf(temp_buffer,sizeof(temp_buffer)-1,"主机'%s'的状态历史",(temp_host->display_name!=NULL)?temp_host->display_name:temp_host->name);
				else
					snprintf(temp_buffer,sizeof(temp_buffer)-1,"主机'%s'上的'%s'服务的状态历史",(temp_service->display_name!=NULL)?temp_service->display_name:temp_service->description,(temp_host->display_name!=NULL)?temp_host->display_name:temp_host->name);
				temp_buffer[sizeof(temp_buffer)-1]='\x0';
				string_width=gdFontSmall->w*strlen(temp_buffer);
				gdImageStringFT(trends_image,&brect[0],color_black,font_file,SMALL_FONT_SIZE,0.0,(drawing_width/2)-(string_width/2)+drawing_x_offset,string_height,(char *)temp_buffer);
				
				
				snprintf(temp_buffer,sizeof(temp_buffer)-1,"%s到%s",start_time,end_time);
				temp_buffer[sizeof(temp_buffer)-1]='\x0';
				string_width=gdFontSmall->w*strlen(temp_buffer);
				gdImageStringFT(trends_image,&brect[0],color_black,font_file,SMALL_FONT_SIZE,0.0,(drawing_width/2)-(string_width/2)+drawing_x_offset, (string_height*2)+5,(char *)temp_buffer);
				
				snprintf(temp_buffer,sizeof(temp_buffer)-1,"状态细分");
				temp_buffer[sizeof(temp_buffer)-1]='\x0';
				string_width=gdFontSmall->w*strlen(temp_buffer);
				gdImageStringFT(trends_image,&brect[0], color_black,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset+drawing_width+15,drawing_y_offset-25,(char *)temp_buffer);
				
				snprintf(temp_buffer,sizeof(temp_buffer)-1," 百分比　     时间合计");
				temp_buffer[sizeof(temp_buffer)-1]='\x0';
				string_width=gdFontSmall->w*strlen(temp_buffer);
				gdImageStringFT(trends_image,&brect[0],color_black,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset+drawing_width+75,drawing_y_offset-string_height+5,(char *)temp_buffer);
				
				draw_line(drawing_x_offset+drawing_width+15,drawing_y_offset-drawing_height+string_height+55,drawing_x_offset+drawing_width+250,drawing_y_offset-drawing_height+string_height+55,color_black);
			}else{
				if(display_type==DISPLAY_HOST_TRENDS)
				...
				snprintf(temp_buffer,sizeof(temp_buffer)-1,"State Breakdowns");
				temp_buffer[sizeof(temp_buffer)-1]='\x0';
				string_width=gdFontSmall->w*strlen(temp_buffer);
				gdImageString(trends_image,gdFontSmall,drawing_x_offset+drawing_width+15,drawing_y_offset-string_height+5,(unsigned char *)temp_buffer,color_black);
	...
	if (display_type == DISPLAY_NO_TRENDS || input_type != GET_INPUT_NONE) {
		...
		if (input_type == GET_INPUT_HOST_TARGET) {
			...
			printf("<DIV CLASS='reportSelectTitle'>步骤２: 主机选择</DIV>\n");
		...
		/* ask the user for what service they want a report for */
			...
			printf("<DIV CLASS='reportSelectTitle'>步骤２: 服务选择</DIV>\n");
		...
		/* ask the user for report range and options */
			...
			printf("<TABLE BORDER=0 CELLPADDING=5 align='center'>\n");
			printf("<tr><td class='reportSelectSubTitle' align=right>报告时间段:</td>\n");
			printf("<td class='reportSelectItem'>\n");
			printf("<select name='timeperiod'>\n");
			...
			printf("<tr><td class='reportSelectSubTitle' align=right>包含软件状态:</td>\n");
			printf("<td class='reportSelectItem'>\n");
			...
			printf("<tr><td class='reportSelectSubTitle' align=right>首先假定%s状态:</td>\n", (display_type == DISPLAY_HOST_TRENDS) ? "主机" : "服务");
			printf("<td class='reportSelectItem'>\n");
			if (display_type == DISPLAY_HOST_TRENDS) {
				printf("<select name='initialassumedhoststate'>\n");
				printf("<option value=%d>未定义\n",AS_NO_DATA);
			...
			printf("<tr><td class='reportSelectSubTitle' align=right>返回存档(用于扫描初始状态):</td>\n");
			printf("<td class='reportSelectItem'>\n");
			printf("<input type='text' name='backtrack' size='2' maxlength='2' value='%d'>\n", backtrack_archives);
			printf("</td></tr>\n");

			printf("<tr><td class='reportSelectSubTitle' align=right>禁止地图:</td><td class='reportSelectItem'><input type='checkbox' name='nomap'></td></tr>");
			printf("<tr><td class='reportSelectSubTitle' align=right>禁止弹出菜单:</td><td class='reportSelectItem'><input type='checkbox' name='nopopups'></td></tr>\n");

			printf("<tr><td class='reportSelectSubTitle' align=right>忽略守护进程启动/重启:</td><td class='reportSelectItem'><input type='checkbox' name='ignorerestart'></td></tr>\n");
			...
			printf("备注：选择禁止地图，报告生成速度大约能提高一倍，但是将不能对时间轴进行缩放.\n");
			printf("</DIV></P>\n");
			*/
		}
		...
		/* as the user whether they want a graph for a host or service */
			...
			printf("<DIV CLASS='reportSelectTitle'>步骤１: 选择报告的类型</DIV>\n");
			...
			printf("<tr><td></td><td class='reportSelectItem'>\n");
			printf("<input type='submit' value='步骤2续'>\n");
			printf("</td></tr>\n");
...
void graph_all_trend_data(void) {
	...
	if (as_list == NULL && current_time > t1 && current_time < (t2 + wobble)) {
		...
		if (display_type == DISPLAY_HOST_TRENDS) {
			if (hststatus != NULL) {
				...
				add_archived_state(last_known_state, AS_HARD_STATE, t1, "假设当前主机状态 (假设日志条目)");
			...
			if (svcstatus != NULL) {
				...
				/* add a dummy archived state item, so something can get graphed */
				add_archived_state(last_known_state, AS_HARD_STATE, t1, "假设当前服务状态 (假设日志条目)");
	...
	if ((display_type == DISPLAY_HOST_TRENDS && initial_assumed_host_state != AS_NO_DATA) || (display_type == DISPLAY_SERVICE_TRENDS && initial_assumed_service_state != AS_NO_DATA)) {
		...
		if (error == FALSE) {
			...
			if (display_type == DISPLAY_HOST_TRENDS)
				add_archived_state(initial_assumed_state, AS_HARD_STATE, initial_assumed_time, "假设首个主机状态 (假设日志条目)");
			else
				add_archived_state(initial_assumed_state, AS_HARD_STATE, initial_assumed_time, "假设首个服务状态 (假设日志条目)");
		}
	}
...
void graph_trend_data(int first_state, int last_state, time_t real_start_time, time_t start_time, time_t end_time, char *state_info) {
	...
	/* else we're creating the HTML, so write map area code... */
		...
		if(display_popups==TRUE){
            ...
            get_time_string(&t1,start_timestring,sizeof(start_timestring)-1,LONG_DATE_TIME); //
			get_time_string(&t2,end_timestring,sizeof(end_timestring)-1,LONG_DATE_TIME); //
			...
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "<B><U>%s</U></B><BR><B>时间范围</B>: <I>%s</I> 到 <I>%s</I><BR><B>持续时间</B>: <I>%02d天%02d时%02d分%02d秒</I><BR><B>状态信息</B>: <I>%s</I>", state_string, start_timestring, end_timestring, days, hours, minutes, seconds, (state_info == NULL) ? "无" : state_info);
			temp_buffer[sizeof(temp_buffer)-1] = '\x0';
...
#ifdef DEBUG
	printf("添加状态 %d @ %s", state_type, ctime(&time_stamp));
#endif
		...
		for (temp_entry = entry_list; temp_entry != NULL; temp_entry = temp_entry->next) {
			...
			if (ignore_daemon_restart == FALSE) {
				...
				if (temp_entry->type == LOGENTRY_STARTUP)
					add_archived_state(AS_PROGRAM_START, AS_NO_DATA, temp_entry->timestamp, "程序启动");
				if (temp_entry->type == LOGENTRY_RESTART)
					add_archived_state(AS_PROGRAM_START, AS_NO_DATA, temp_entry->timestamp, "程序重启");

				/* program stops */
				if (temp_entry->type == LOGENTRY_SHUTDOWN)
					add_archived_state(AS_PROGRAM_END, AS_NO_DATA, temp_entry->timestamp, "正常的程序终止");
				if (temp_entry->type == LOGENTRY_BAILOUT)
					add_archived_state(AS_PROGRAM_END, AS_NO_DATA, temp_entry->timestamp, "异常的程序终止");
			}
...
void draw_timestamp(int ts_pixel, time_t ts_time) {
	char temp_buffer[MAX_INPUT_BUFFER];
    char timestring_short[MAX_INPUT_BUFFER]; //
	int string_height;
	int string_width;

    get_time_string(&ts_time,timestring_short,sizeof(timestring_short)-1,GRAPH_DATE_TIME);	//
	snprintf(temp_buffer,sizeof(temp_buffer)-1,"%s",timestring_short );	//
...
void draw_time_breakdowns(void) {
	...
	if (display_type == DISPLAY_HOST_TRENDS) {
        ...
        /*** Image TrueTypeFont ***/
        if( (strlen(font_file) > 5) && (access(font_file, F_OK) == 0) ){
            get_time_breakdown_string(total_time,time_up,"运行",&temp_buffer[0],sizeof(temp_buffer));
            gdImageStringFT(trends_image,&brect[0],color_darkgreen,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset+drawing_width+20,drawing_y_offset+15,(char *)temp_buffer);
            ...
            gdImageStringFT(trends_image,&brect[0],color_darkpink,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset-10-(gdFontSmall->w*7),drawing_y_offset+55,(char *)"不可达");
            
            get_time_breakdown_string(total_time,time_indeterminate,"不确定",&temp_buffer[0],sizeof(temp_buffer));
            gdImageStringFT(trends_image,&brect[0],color_black,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset+drawing_width+20,drawing_y_offset+75,(char *)temp_buffer);
            gdImageStringFT(trends_image,&brect[0],color_black,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset-10-(gdFontSmall->w*7),drawing_y_offset+75,(char *)"不确定");
        }else{
        ...
        if( (strlen(font_file) > 5) && (access(font_file, F_OK) == 0) ){
            ...
            gdImageStringFT(trends_image,&brect[0],color_darkgreen,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset+drawing_width+20,drawing_y_offset+15,(char *)temp_buffer);
            gdImageStringFT(trends_image,&brect[0],color_darkgreen,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset-10-(gdFontSmall->w*5),drawing_y_offset+15,(char *)"正常");
            
            get_time_breakdown_string(total_time,time_warning,"警报",&temp_buffer[0],sizeof(temp_buffer));
            gdImageStringFT(trends_image,&brect[0],color_yellow,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset+drawing_width+20,drawing_y_offset+35,(char *)temp_buffer);
            gdImageStringFT(trends_image,&brect[0],color_yellow,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset-10-(gdFontSmall->w*5),drawing_y_offset+35,(char *)"警报");
            ...
            gdImageStringFT(trends_image,&brect[0],color_black,font_file,SMALL_FONT_SIZE,0.0,drawing_x_offset-10-(gdFontSmall->w*7),drawing_y_offset+95,(char *)"不确定");
        }else{
...
void get_time_breakdown_string(unsigned long total_time, unsigned long state_time, char *state_string, char *buffer, int buffer_length) {
	...
	snprintf(buffer, buffer_length - 1, "%-13s: (%.3f%%) %02d天%02d时%02d分%02d秒", state_string, percent_time, days, hours, minutes, seconds);
...
void draw_line(int x1,int y1,int x2,int y2,int color){
	...
	gdImageSetStyle(trends_image,styleSolid,1);
