...
int main(void) {
	...
	document_header(CGI_ID, TRUE, "日志文件");
	...
	if (display_header == TRUE) {
		...
		display_info_table("事件日志", &current_authdata, daemon_check);
		printf("</td>\n");
	...
	if (is_authorized_for_system_information(&current_authdata) == FALSE) {
		print_generic_error_message("很显然您无权查看日志文件...", "如果您认为这是一个错误,请检查HTTP服务器关于该CGI的访问权限设置.", 0);
		return ERROR;
	}
...
void display_logentries() {
	...
	/* now we start displaying the log entries */
		...
		if (content_type == JSON_CONTENT) {
			...
			printf("%s时间戬%s%s", csv_data_enclosure, csv_data_enclosure, csv_delimiter);
			printf("%s日期时间%s%s", csv_data_enclosure, csv_data_enclosure, csv_delimiter);
		...
		for (temp_entry = entry_list; temp_entry != NULL; temp_entry = temp_entry->next) {
			...
			get_time_string(&temp_entry->timestamp,current_message_date,sizeof(current_message_date)-1,LOG_DATE); //
			...
			if (content_type == JSON_CONTENT) {
				...
				if (json_start == FALSE)
					printf(",\n");
				json_start = FALSE;
				printf("{ \"时间戬\": %lu, ", temp_entry->timestamp);
				printf(" \"日期时间\": \"%s\", ", date_time);
				printf(" \"日志条目\": \"%s\"}", json_encode(temp_entry->entry_text));
			} else if (content_type == CSV_CONTENT) {
	...
	if (user_has_seen_something == FALSE && content_type != CSV_CONTENT && content_type != JSON_CONTENT)
		printf("<DIV CLASS='warningMessage'>没有发现日志条目!</DIV>");
...
void show_filter(void) {
	...
	printf("<tr><td valign=top align=right style='padding-right:21.5em;'>过滤:&nbsp;&nbsp;");
	...
	/* search box */
	printf("<tr><td align=right width='10%%'>搜索:</td>");
	...
	printf("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' value='清除' onClick=\"document.getElementById('query_string').value = '';\"></td></tr>");
	...
	printf("<tr><td>开始:&nbsp;&nbsp;</td><td><INPUT TYPE='TEXT' class='timepicker' NAME='start_time' VALUE='%s' SIZE=\"25\"></td></tr>", buffer);

	get_time_string(&ts_end, buffer, sizeof(buffer) - 1, SHORT_DATE_TIME);
	printf("<tr><td>结束:&nbsp;&nbsp;</td><td><INPUT TYPE='TEXT' class='timepicker' NAME='end_time' VALUE='%s' SIZE=\"25\"></td></tr></table></div>", buffer);

	printf("</td></tr>\n");

	/* Filter Entry types */
	printf("<tr><td>条目类型:</td><td>\n");
	printf("<table border=0 cellspacing=0 cellpadding=0>\n");
	printf("<tr><td align=center>打开</td><td align=center>关闭</td><td>类型</td></tr>\n");
	printf("<tr><td><input type=radio name='noti' value=打开 %s></td><td><input type=radio name='noti' value=关闭 %s></td><td>通知</td></tr>\n", (show_notifications == TRUE) ? "checked" : "", (show_notifications == FALSE) ? "checked" : "");
	printf("<tr><td><input type=radio name='hst' value=打开 %s></td><td><input type=radio name='hst' value=关闭 %s></td><td>主机状态</td></tr>\n", (show_host_status == TRUE) ? "checked" : "", (show_host_status == FALSE) ? "checked" : "");
	printf("<tr><td><input type=radio name='sst' value=打开 %s></td><td><input type=radio name='sst' value=关闭 %s></td><td>服务状态</td></tr>\n", (show_service_status == TRUE) ? "checked" : "", (show_service_status == FALSE) ? "checked" : "");
	printf("<tr><td><input type=radio name='cmd' value=打开 %s></td><td><input type=radio name='cmd' value=关闭 %s></td><td>额外命令</td></tr>\n", (show_external_commands == TRUE) ? "checked" : "", (show_external_commands == FALSE) ? "checked" : "");
	printf("<tr><td><input type=radio name='sms' value=打开 %s></td><td><input type=radio name='sms' value=关闭 %s></td><td>系统信息</td></tr>\n", (show_system_messages == TRUE) ? "checked" : "", (show_system_messages == FALSE) ? "checked" : "");
	printf("<tr><td><input type=radio name='evh' value=打开 %s></td><td><input type=radio name='evh' value=关闭 %s></td><td>事件处理</td></tr>\n", (show_event_handler == TRUE) ? "checked" : "", (show_event_handler == FALSE) ? "checked" : "");
	printf("<tr><td><input type=radio name='flp' value=打开 %s></td><td><input type=radio name='flp' value=关闭 %s></td><td>抖动</td></tr>\n", (show_flapping == TRUE) ? "checked" : "", (show_flapping == FALSE) ? "checked" : "");
	printf("<tr><td><input type=radio name='dwn' value=打开 %s></td><td><input type=radio name='dwn' value=关闭 %s></td><td>宕机</td></tr>\n", (show_downtime == TRUE) ? "checked" : "", (show_downtime == FALSE) ? "checked" : "");
