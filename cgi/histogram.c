...
int main(int argc, char **argv) {
	...
	document_header(CGI_ID, TRUE, "柱状图");
	...
	if (content_type == HTML_CONTENT && display_header == TRUE) {
		...
		if (display_type == DISPLAY_HOST_HISTOGRAM)
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "主机警告柱状图");
		else if (display_type == DISPLAY_SERVICE_HISTOGRAM)
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "服务警告柱状图");
		else
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "主机和服务警告柱状图");
		...
		if (display_type != DISPLAY_NO_HISTOGRAM && input_type == GET_INPUT_NONE) {
			...
			printf("<IMG SRC='%s%s' BORDER=0 ALT='%s事件柱状图' TITLE='%s事件柱状图'>\n", url_images_path, TRENDS_ICON, (display_type == DISPLAY_HOST_HISTOGRAM) ? "主机" : "服务", (display_type == DISPLAY_HOST_HISTOGRAM) ? "主机" : "服务");
		...
		if (display_type != DISPLAY_NO_HISTOGRAM && input_type == GET_INPUT_NONE) {
			...
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>细分类型:</td><td CLASS='optBoxItem' valign=top align=left>初始状态记录:</td></tr>\n");
			...
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>报告时间段:</td><td CLASS='optBoxItem' valign=top align=left>假定状态保持:</td></tr>\n");
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>\n");
			...
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>状态类型图:</td><td CLASS='optBoxItem' valign=top align=left></td></tr>\n");
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>\n");
	...
	if (is_authorized == FALSE) {
		...
		if (content_type == HTML_CONTENT) {
			if (display_type == DISPLAY_HOST_HISTOGRAM)
				print_generic_error_message("很显然您无权获取指定的主机信息...", NULL, 0);
			else
				print_generic_error_message("很显然您无权获取指定的服务信息...", NULL, 0);
		}
	...
	if (display_type == DISPLAY_NO_HISTOGRAM || input_type != GET_INPUT_NONE) {
		...
		/* ask the user for report range and options */
			...
			printf("<option value=last24hours>过去24小时\n");
			...
			printf("<tr><td class='reportSelectSubTitle' align=right>统计数据细分:</td>\n");
			printf("<td class='reportSelectItem'>\n");
			printf("<select name='breakdown'>\n");
			printf("<option value=monthly>按月\n");
			printf("<option value=dayofmonth SELECTED>按天\n");
			printf("<option value=dayofweek>按周\n");
			printf("<option value=hourly>按小时\n");
			printf("</select>\n");
			printf("</td></tr>\n");

			printf("<tr><td class='reportSelectSubTitle' align=right>事件图:</td>\n");
			printf("<td class='reportSelectItem'>\n");
			printf("<select name='graphevents'>\n");
			if (display_type == DISPLAY_HOST_HISTOGRAM) {
				printf("<option value=%d %s>所有主机事件\n",GRAPH_HOST_ALL,(graph_events==GRAPH_HOST_ALL)?"SELECTED":"");
			...
			printf("<tr><td class='reportSelectSubTitle' align=right>状态类型图:</td>\n");
			printf("<td class='reportSelectItem'>\n");
			printf("<select name='graphstatetypes'>\n");
			printf("<option value=%d>硬件状态\n",GRAPH_HARD_STATETYPES);
			printf("<option value=%d>软件状态\n",GRAPH_SOFT_STATETYPES);
			printf("<option value=%d SELECTED>硬件和软件状态\n", GRAPH_ALL_STATETYPES);
			printf("</select>\n");
			printf("</td></tr>\n");
...
void graph_all_histogram_data(void) {
	...
	*/
    ...
    if( (strlen(font_file) > 5) && (access(font_file, F_OK) == 0) ){
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"事件数");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], color_black, font_file, SMALL_FONT_SIZE,0.0,DRAWING_X_OFFSET-(string_width*4), DRAWING_Y_OFFSET-(DRAWING_HEIGHT/2)-15, (char *)temp_buffer);
        
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"事件数");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], color_black, font_file, SMALL_FONT_SIZE,0.0,DRAWING_X_OFFSET-(string_width*4), DRAWING_Y_OFFSET-(DRAWING_HEIGHT/2)+5, (char *)temp_buffer);
	}else{
	...
	if( (strlen(font_file) > 5) && (access(font_file, F_OK) == 0) ){
		if(breakdown_type==BREAKDOWN_MONTHLY)
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"按月");	
		else if(breakdown_type==BREAKDOWN_DAY_OF_MONTH)
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"按天");
		else if(breakdown_type==BREAKDOWN_DAY_OF_WEEK)
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"按周");
		else
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"按小时(15分钟间隔)");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		if( (breakdown_type==BREAKDOWN_MONTHLY) || (breakdown_type==BREAKDOWN_DAY_OF_MONTH) || (breakdown_type==BREAKDOWN_DAY_OF_WEEK) ){
			gdImageStringFT(histogram_image, &brect[0], color_black, font_file, SMALL_FONT_SIZE,0.0,DRAWING_X_OFFSET+(DRAWING_WIDTH/2)-(string_width/2), DRAWING_Y_OFFSET+40, (char *)temp_buffer);
		}else{
			gdImageStringFT(histogram_image, &brect[0], color_black, font_file, SMALL_FONT_SIZE,0.0,DRAWING_X_OFFSET+(DRAWING_WIDTH/2)-(string_width/2), DRAWING_Y_OFFSET+60, (char *)temp_buffer);
        }
	}else{
	    if(breakdown_type==BREAKDOWN_MONTHLY)
    ...
    if( (strlen(font_file) > 5) && (access(font_file, F_OK) == 0) ){
        ...
        if(display_type==DISPLAY_HOST_HISTOGRAM)
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"主机'%s'的历史事件",(temp_host->display_name!=NULL)?temp_host->display_name:temp_host->name);
		else
			snprintf(temp_buffer,sizeof(temp_buffer)-1,"主机'%s'上的'%s'服务的历史事件",(temp_service->display_name!=NULL)?temp_service->display_name:temp_service->description,(temp_host->display_name!=NULL)?temp_host->display_name:temp_host->name);
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0],color_black,font_file,SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+(DRAWING_WIDTH/2)-(string_width/2),string_height-2,(char *)temp_buffer);
		
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"%s到%s",start_time,end_time);
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer); 
		gdImageStringFT(histogram_image, &brect[0],color_black,font_file,SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+(DRAWING_WIDTH/2)-(string_width/2),string_height+16,(char *)temp_buffer);
	}else{
	...
	/* graph stats */
    if( (strlen(font_file) > 5) && (access(font_file, F_OK) == 0) ){
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"事件类型");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], color_black, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+15,DRAWING_Y_OFFSET-DRAWING_HEIGHT-10,(char *)temp_buffer);
		
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"  最小  最大  合计     平均");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], color_black, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+100,DRAWING_Y_OFFSET-DRAWING_HEIGHT+10,(char *)temp_buffer);
		...
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"恢复(%s):",(display_type==DISPLAY_SERVICE_HISTOGRAM)?"正常":"运行");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], color_green, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+15,DRAWING_Y_OFFSET-DRAWING_HEIGHT+((string_height+5)*2),(char *)temp_buffer);
		...
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], color_black, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+105,DRAWING_Y_OFFSET-DRAWING_HEIGHT+((string_height+5)*2),(char *)temp_buffer);
		
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"%s:",(display_type==DISPLAY_SERVICE_HISTOGRAM)?"警报":"宕机");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], (display_type==DISPLAY_SERVICE_HISTOGRAM)?color_yellow:color_red, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+15,DRAWING_Y_OFFSET-DRAWING_HEIGHT+((string_height+5)*3),(char *)temp_buffer);
		...
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], color_black, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+105,DRAWING_Y_OFFSET-DRAWING_HEIGHT+((string_height+5)*3),(char *)temp_buffer);
		
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"%s:",(display_type==DISPLAY_SERVICE_HISTOGRAM)?"未知":"不可达");
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], color_darkpink, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+15,DRAWING_Y_OFFSET-DRAWING_HEIGHT+((string_height+5)*4),(char *)temp_buffer);
		...
		temp_buffer[sizeof(temp_buffer)-1]='\x0';
		string_width=gdFontSmall->w*strlen(temp_buffer);
		gdImageStringFT(histogram_image, &brect[0], color_black, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+105,DRAWING_Y_OFFSET-DRAWING_HEIGHT+((string_height+5)*4),(char *)temp_buffer);
		
		if(display_type==DISPLAY_SERVICE_HISTOGRAM){
			
            snprintf(temp_buffer,sizeof(temp_buffer)-1,"严重:");
            temp_buffer[sizeof(temp_buffer)-1]='\x0';
            string_width=gdFontSmall->w*strlen(temp_buffer);
            gdImageStringFT(histogram_image, &brect[0],color_red, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+15,DRAWING_Y_OFFSET-DRAWING_HEIGHT+((string_height+5)*5),(char *)temp_buffer);
            ...
            temp_buffer[sizeof(temp_buffer)-1]='\x0';
            string_width=gdFontSmall->w*strlen(temp_buffer);
            gdImageStringFT(histogram_image, &brect[0],color_black, font_file, SMALL_FONT_SIZE, 0.0, DRAWING_X_OFFSET+DRAWING_WIDTH+105,DRAWING_Y_OFFSET-DRAWING_HEIGHT+((string_height+5)*5),(char *)temp_buffer);
        }
	}else{
		...
		for (temp_entry = entry_list; temp_entry != NULL; temp_entry = temp_entry->next) {
			...
			if (display_type == DISPLAY_HOST_HISTOGRAM) {
				...
				case LOGENTRY_HOST_UP:
					...
					if (!(graph_statetypes & GRAPH_SOFT_STATETYPES) && strstr(temp_entry->entry_text, ";软件状态;"))
						continue;

					/* skip hard states if necessary */
					if (!(graph_statetypes & GRAPH_HARD_STATETYPES) && strstr(temp_entry->entry_text, ";硬件状态;"))
						continue;
