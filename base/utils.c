...
int my_system_r(icinga_macros *mac, char *cmd, int timeout, int *early_timeout, double *exectime, char **output, int max_output_length) {
	...
	*exectime = 0.0;
	...
	log_debug_info(DEBUGL_COMMANDS, 1, "运行命令 '%s'...\n", cmd);
	...
	if (use_epn == TRUE) {
		...
		if (SvTRUE(ERRSV)) {
			...
			log_debug_info(DEBUGL_COMMANDS, 0, "编译%s嵌入Perl失败, 编译错误%s\n", fname, temp_buffer);
			...
			log_debug_info(DEBUGL_COMMANDS, 0, "编译%s嵌入Perl成功和传回插件处理函数(Perl子程序代码号)\n", fname);
	...
	if (pid == -1) {
		logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: fork() in my_system_r() failed for command \"%s\"\n", cmd);
	...
	/* parent waits for child to finish executing command */
		...
		*exectime = (double)((double)(end_time.tv_sec - start_time.tv_sec) + (double)((end_time.tv_usec - start_time.tv_usec) / 1000) / 1000.0);
		...
		if (result == 126 || result == 127) {
			logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: 试图执行命令 \"%s\" 结果,返回%d代码. 确保您尝试执行脚本或二进制确实存在...\n", cmd, result);
		}
...
int check_time_against_period(time_t test_time, timeperiod *tperiod) {
	...
	for (daterange_type = 0; daterange_type < DATERANGE_TYPES; daterange_type++) {
		...
		for (temp_daterange = tperiod->exceptions[daterange_type]; temp_daterange != NULL; temp_daterange = temp_daterange->next) {
			...
			printf("类型: %d\n", daterange_type);
			printf("测试:     %lu = %s", (unsigned long)test_time, ctime(&test_time));
			printf("午夜: %lu = %s", (unsigned long)midnight, ctime(&midnight));
#endif
...
#ifdef TEST_TIMEPERIODS_A
			printf("开始:    %lu = %s", (unsigned long)start_time, ctime(&start_time));
			printf("结束:      %lu = %s", (unsigned long)end_time, ctime(&end_time));
#endif
...
#ifdef TEST_TIMEPERIODS_A
			printf("新开始:    %lu = %s", (unsigned long)start_time, ctime(&start_time));
			printf("新结束:      %lu = %s", (unsigned long)end_time, ctime(&end_time));
			printf("已过%d天\n", days);
			printf("夏令时SHIFT键:   %d", shift);
#endif
...
#ifdef TEST_TIMEPERIODS_A
					printf("  范围开始: %lu (%lu) = %s", temp_timerange->range_start, (unsigned long)day_range_start, ctime(&day_range_start));
					printf("  范围结束:   %lu (%lu) = %s", temp_timerange->range_end, (unsigned long)day_range_end, ctime(&day_range_end));
#endif
...
#ifdef TEST_TIMEPERIODS_B
	printf("首选时间:    %lu = %s", (unsigned long)preferred_time, ctime(&preferred_time));
	printf("当前时间: %lu = %s", (unsigned long)current_time, ctime(&current_time));
	printf("首选年份:    %d, 月: %d, 天: %d, 星期: %d\n", pref_time_year, pref_time_mon, pref_time_mday, pref_time_wday);
	printf("当前年份: %d, 月: %d, 天: %d, 星期: %d\n", current_time_year, current_time_mon, current_time_mday, current_time_wday);
#endif
...
#ifdef TEST_TIMEPERIODS_B
			printf("开始时间: %lu = %s", start_time, ctime(&start_time));
#endif
...
#ifdef TEST_TIMEPERIODS_B
			printf("开始时间: %lu = %s", (unsigned long)start_time, ctime(&start_time));
			printf("结束时间1: %lu = %s", (unsigned long)end_time, ctime(&end_time));
#endif
...
#ifdef TEST_TIMEPERIODS_B
			printf("结束时间2: %lu = %s", (unsigned long)end_time, ctime(&end_time));
#endif
...
#ifdef TEST_TIMEPERIODS_B
					printf("午夜: %lu = %s", midnight, ctime(&midnight));
...
#ifdef TEST_TIMEPERIODS_B
			printf("\n开始:     %lu = %s", (unsigned long)start_time, ctime(&start_time));
			printf("结束:       %lu = %s", (unsigned long)end_time, ctime(&end_time));
			printf("首选: %lu = %s", (unsigned long)preferred_time, ctime(&preferred_time));
			printf("当前:   %lu = %s", (unsigned long)current_time, ctime(&current_time));
#endif
...
#ifdef TEST_TIMEPERIODS_B
						printf("    最早时间: %lu = %s", (unsigned long)earliest_time, ctime(&earliest_time));
#endif
					}
				}
			}
...
void sighandler(int sig) {
	...
	if (sig == SIGSEGV)
		logit(NSLOG_PROCESS_INFO, TRUE, "Caught SIG%s, 关机...\n", sigs[sig]);
...
int daemon_init(void) {
	...
	if (lockfile < 0) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "在文件%s上无法获得: %s\n", lock_file, strerror(errno));
		logit(NSLOG_PROCESS_INFO | NSLOG_RUNTIME_ERROR, TRUE, "救助尝试守护进程化时遇到的错误... (PID=%d)", (int)getpid());
	...
	if ((val = read(lockfile, buf, (size_t)10)) < 0) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "Lockfile存在，但无法读取");
		cleanup();
		exit(ERROR);
	}
	...
	if (val > 0) {
		if ((val = sscanf(buf, "%d", &pidno)) < 1) {
			logit(NSLOG_RUNTIME_ERROR, TRUE, "Lockfile '%s' 不包含有效的PID (%s)", lock_file, buf);
			cleanup();
			exit(ERROR);
		}
	}
	...
	if (fcntl(lockfile, F_SETLK, &lock) < 0) {
		if (errno == EACCES || errno == EAGAIN) {
			...
			logit(NSLOG_RUNTIME_ERROR, TRUE, "Lockfile '%s' 看起来其已持有另一%s实例 (PID %d).  救助...", PROGRAM_NAME, lock_file, (int)lock.l_pid);
		} else
			logit(NSLOG_RUNTIME_ERROR, TRUE, "无法查看lockfile '%s': %s. 救助...", lock_file, strerror(errno));
...
int drop_privileges(char *user, char *group) {
	...
	if (group != NULL) {
		...
		if (strspn(group, "0123456789") < strlen(group)) {
			...
			if (grp != NULL)
				...
				logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: 无法获取群组项目 '%s'", group);
		}
	...
	if (user != NULL) {
		...
		if (strspn(user, "0123456789") < strlen(user)) {
			...
			if (pw != NULL)
				...
				logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: 无法获取passwd条目 '%s'", user);
		}
	...
	if (gid != getegid()) {
		...
		if (setgid(gid) == -1) {
			logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: 无法制定有效的 GID=%d", (int)gid);
			result = ERROR;
		}
	}
	...
	if (uid != geteuid()) {
			...
			if (initgroups(user, gid) == -1) {
				if (errno == EPERM)
					logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: 无法更改附加组使用 initgroups() -- 我希望知道你在做什么");
				else {
					logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: 可能是root用户没有丢弃使用initgroups()权限");
					return ERROR;
				}
			}
		}
	...
	if (setuid(uid) == -1) {
		logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: 无法制定有效的 UID=%d", (int)uid);
		result = ERROR;
	}

	log_debug_info(DEBUGL_PROCESS, 0, "新 UID/GID: %d/%d\n", (int)getuid(), (int)getgid());
...
int move_check_result_to_queue(char *checkresult_file) {
	...
	if (output_file_fd >= 0) {
		...
		log_debug_info(DEBUGL_CHECKS, 2, "移动临时检查结果文件 '%s' 到队列文件 '%s'...\n", checkresult_file, output_file);
	...
	if (result != 0)
		logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: 无法移动文件 '%s' 到检查结果队列.\n", checkresult_file);
...
int process_check_result_queue(char *dirname) {
	...
	if (dirname == NULL) {
		logit(NSLOG_CONFIG_ERROR, TRUE, "错误: 无检查结果队列指定的目录.\n");
		return ERROR;
	}

	/* open the directory for reading */
	if ((dirp = opendir(dirname)) == NULL) {
		logit(NSLOG_CONFIG_ERROR, TRUE, "错误: 无法打开检查结果队列目录并读取'%s'.\n", dirname);
		return ERROR;
	}

	log_debug_info(DEBUGL_CHECKS, 1, "开始读取检查结果队列 '%s'...\n", dirname);
	...
	while ((dirfile = readdir(dirp)) != NULL) {
		...
		if (x == 7 && dirfile->d_name[0] == 'c') {
			...
			if (stat(file, &stat_buf) == -1) {
				logit(NSLOG_RUNTIME_WARNING, TRUE, "警报: 无法stat()检查结果文件'%s'.\n", file);
				continue;
			}
...
int process_check_result_file(char *fname) {
	...
	log_debug_info(DEBUGL_CHECKS, 1, "处理检查结果文件: '%s'\n", fname);
...
int open_command_file(void) {
    ...
    if((result=mkfifo(command_file,S_IRWXU | S_IRWXG | S_IRWXO))!=0) {

			logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 无法创建额外命令文件 '%s' as named pipe: (%d) -> %s.  If this file already exists and you are sure that another copy of %s is not running, you should delete this file.\n", command_file, errno, strerror(errno), PROGRAM_NAME);
			return ERROR;
		}
	}
	...
	if ((command_file_fd = open(command_file, O_RDWR | O_NONBLOCK)) < 0) {
		...
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 使用 open() 无法打开并读取额外命令文件: (%d) -> %s\n", errno, strerror(errno));
	...
	if ((command_file_fp = (FILE *)fdopen(command_file_fd, "r")) == NULL) {
		...
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 使用 fdopen() 无法打开并读取额外命令文件: (%d) -> %s\n", errno, strerror(errno));
	...
	if (init_command_file_worker_thread() == ERROR) {
		...
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 无法初始化命令文件的工作线程.\n");
	...
	if (rename_result == -1) {
		...
		if (errno == EXDEV) {
			...
			if (my_fcopy(source, dest) == ERROR) {
				logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 无法重命名文件 '%s' 为 '%s': %s\n", source, dest, strerror(errno));
				return -1;
			}
...
int my_fdcopy(char *source, char *dest, int dest_fd) {
	...
	if ((source_fd = open(source, O_RDONLY, 0644)) < 0) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 无法打开文件并读取 '%s' : %s\n", source, strerror(errno));
		return ERROR;
	}
	...
	if (fstat(source_fd, &st) < 0) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: Unable to stat source file '%s' for my_fcopy(): %s\n", source, strerror(errno));
		close(source_fd);
		return ERROR;
	}
	...
	while (tot_written < st.st_size) {
		...
		if (rd_result < 0) {
			if (errno == EAGAIN || errno == EINTR)
				continue;
			logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: my_fcopy() failed to read from '%s': %s\n", source, strerror(errno));
			break;
		}
		tot_read += rd_result;
		...
		while (loop_wr < rd_result) {
			...
			if (wr_result < 0) {
				if (errno == EAGAIN || errno == EINTR)
					continue;
				logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: my_fcopy() 无法写入 '%s': %s\n", dest, strerror(errno));
				break;
			}
			loop_wr += wr_result;
		}
...
int my_fcopy(char *source, char *dest) {
	...
	if ((dest_fd = open(dest, O_WRONLY | O_TRUNC | O_CREAT | O_APPEND, 0644)) < 0) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 无法打开并写入 '%s' : %s\n", dest, strerror(errno));
		return ERROR;
	}
