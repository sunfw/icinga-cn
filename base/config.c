...
int read_main_config_file(char *main_config_file) {
	...
	if ((thefile = mmap_fopen(main_config_file)) == NULL) {
		logit(NSLOG_CONFIG_ERROR, TRUE, "错误: 无法打开并读取主配置文件'%s'!", main_config_file);
		return ERROR;
	}
	...
	while (1) {
		...
		if (!strcmp(variable, "resource_file")) {
			...
			if(read_resource_file(value) == ERROR) {
                                dummy = asprintf(&error_message, "资源文件解析失败");
				error = TRUE;
				break;
			}
		}
			...
			if (strlen(value) > MAX_FILENAME_LENGTH - 1) {
				dummy = asprintf(&error_message, "日志文件太长");
				error = TRUE;
				break;
			}
			...
			if (strlen(value) > MAX_FILENAME_LENGTH - 1) {
				dummy = asprintf(&error_message, "调试日志文件太长");
				error = TRUE;
				break;
			}
			...
			if (strlen(value) > MAX_FILENAME_LENGTH - 1) {
				dummy = asprintf(&error_message, "命令文件太长");
				error = TRUE;
				break;
			}
			...
			if (strlen(value) > MAX_FILENAME_LENGTH - 1) {
				dummy = asprintf(&error_message, "临时文件太长");
				error = TRUE;
				break;
			}
			...
			if (strlen(value) > MAX_FILENAME_LENGTH - 1) {
				dummy = asprintf(&error_message, "临时路径太长");
				error = TRUE;
				break;
			}

			if ((tmpdir = opendir((char *)value)) == NULL) {
				dummy = asprintf(&error_message, "临时路径不是有效的目录");
				error = TRUE;
				break;
			}
			closedir(tmpdir);
			...
			if (strlen(value) > MAX_FILENAME_LENGTH - 1) {
				dummy = asprintf(&error_message, "检查结果路径太长");
				error = TRUE;
				break;
			}

			if ((tmpdir = opendir((char *)value)) == NULL) {
				dummy = asprintf(&error_message, "检查结果路径不是有效目录");
				error = TRUE;
				break;
			}
			closedir(tmpdir);
			...
			if (strlen(value) > MAX_FILENAME_LENGTH - 1) {
				dummy = asprintf(&error_message, "锁定文件太长");
				error = TRUE;
				break;
			}
			...
			logit(NSLOG_CONFIG_WARNING, TRUE, "警报: log_external_commands_user variable ignored. This has been deprecated.");
		}
			...
			logit(NSLOG_CONFIG_WARNING, TRUE, "警报: aggregate_status_updates directive ignored.  All status file updates are now aggregated.");
		}
			...
			logit(NSLOG_CONFIG_WARNING, TRUE, "警报: 忽略check_for_updates变量. Icinga核心不支持程序更新检查");
		}

		else if (!strcmp(variable, "bare_update_check")) {
			/* ignore it for compatibility reasons */
			logit(NSLOG_CONFIG_WARNING, TRUE, "警报: 忽略bare_update_check变量. Icinga核心不支持程序更新检查");
		}
		...
		/*** AUTH_FILE VARIABLE USED BY EMBEDDED PERL INTERPRETER ***/
			...
			if (strlen(value) > MAX_FILENAME_LENGTH - 1) {
				dummy = asprintf(&error_message, "认证文件太长");
				error = TRUE;
				break;
			}
		...
		/* warn about old variables */
			...
			logit(NSLOG_CONFIG_WARNING, TRUE, "警报: comment_file variable ignored.  Comments are now stored in the status and retention files.");
		} else if (!strcmp(variable, "downtime_file") || !strcmp(variable, "xdddefault_downtime_file")) {
			logit(NSLOG_CONFIG_WARNING, TRUE, "警报: downtime_file variable ignored.  Downtime entries are now stored in the status and retention files.");
		}
	...
	/* handle errors */
	if (error == TRUE) {
		logit(NSLOG_CONFIG_ERROR, TRUE, "错误发生在配置文件 '%s' - 行 %d (%s)", main_config_file, current_line, (error_message == NULL) ? "NULL" : error_message);
		return ERROR;
	}
	...
	if (!strcmp(log_file, "")) {
		if (daemon_mode == FALSE)
			printf("错误: Log file is not specified anywhere in main config file '%s'!\n", main_config_file);
		return ERROR;
	}
...
int read_resource_file(char *resource_file) {
	...
	if ((thefile = mmap_fopen(resource_file)) == NULL) {
		logit(NSLOG_CONFIG_ERROR, TRUE, "错误: 无法打开并读取资源文件 '%s' !", resource_file);
		return ERROR;
	}
	...
	while (1) {
		...
		/* get the variable name */
		if ((temp_ptr = my_strtok(input, "=")) == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE, "错误: 空的变量 - 资源文件'%s'的%d行", resource_file, current_line);
			error = TRUE;
			break;
		}
		if ((variable = (char *)strdup(temp_ptr)) == NULL) {
		...
		/* get the value */
		if ((temp_ptr = my_strtok(NULL, "\n")) == NULL) {
			logit(NSLOG_CONFIG_ERROR, TRUE, "错误: 空的变量值 - 资源文件'%s'的%d行", resource_file, current_line);
			error = TRUE;
			break;
		}
		if ((value = (char *)strdup(temp_ptr)) == NULL) {
...
int pre_flight_check(void) {
	...
	if (global_host_event_handler != NULL) {
		...
		if (temp_command == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: 没有定义全局主机事件的处理命令 '%s'!", temp_command_name);
			errors++;
		}
	...
	if (global_service_event_handler != NULL) {
		...
		temp_command = find_command(temp_command_name);
		if (temp_command == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: 没有定义全局服务事件的处理命令 '%s'!", temp_command_name);
			errors++;
		}
	...
	if (ocsp_command != NULL) {
		...
		temp_command = find_command(temp_command_name);
		if (temp_command == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: 没有定义强迫性处理命令 '%s'!", temp_command_name);
			errors++;
		}
	...
	if (ochp_command != NULL) {
		...
		temp_command = find_command(temp_command_name);
		if (temp_command == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误:没有定义强迫性主机处理命令 '%s' !", temp_command_name);
			errors++;
		}
	...
	if ((temp_path_fd = mkstemp(buf)) == -1) {
		logit(NSLOG_VERIFICATION_ERROR, TRUE, "\t错误: 无法写入temp_path ('%s') - %s\n", temp_path, strerror(errno));
		errors++;
	} else {
		close(temp_path_fd);
		remove(buf);
	...
	if (illegal_output_chars == NULL) {
		logit(NSLOG_VERIFICATION_WARNING, TRUE, "%s", "警报: Nothing specified for illegal_macro_output_chars variable!\n");
		warnings++;
	}
	else {
		char *p;
	...
	if (test_scheduling == TRUE) {
		...
		printf("CONFIG验证时间          (* = Potential for speedup with -x option)\n");
		printf("----------------------------------\n");
		printf("对象关系: %.6lf 秒\n", runtime[0]);
		printf("圆弧路径:       %.6lf 秒  *\n", runtime[1]);
		printf("杂项:                 %.6lf 秒\n", runtime[2]);
		printf("                      ============\n");
		printf("总计:                %.6lf 秒  * = %.6lf sec (%.1f%%) estimated savings\n", runtime[3], runtime[1], (runtime[1] / runtime[3]) * 100.0);
		printf("\n\n");
	}
...
int pre_flight_object_check(int *w, int *e) {
	...
	if (get_service_count() == 0) {
		logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: There are no services defined!");
		warnings++;
	}
	total_objects = 0;
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		if (!temp_host) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: 在服务 '%s' 中没有指定主机 '%s' 的任何定义!", temp_service->description, temp_service->host_name);
			errors++;
		}
		...
		if (temp_service->event_handler != NULL) {
			...
			temp_command = find_command(temp_command_name);
			if (temp_command == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Event handler command '%s' specified in service '%s' for host '%s' not defined anywhere", temp_command_name, temp_service->description, temp_service->host_name);
				errors++;
			}
		...
		temp_command = find_command(temp_command_name);
		if (temp_command == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Service check command '%s' specified in service '%s' for host '%s' not defined anywhere!", temp_command_name, temp_service->description, temp_service->host_name);
			errors++;
		}
		...
		if (temp_service->notify_on_recovery == TRUE && temp_service->notify_on_warning == FALSE && temp_service->notify_on_critical == FALSE) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Recovery notification option in service '%s' for host '%s' doesn't make any sense - specify warning and/or critical options as well", temp_service->description, temp_service->host_name);
			warnings++;
		}
		...
		for (temp_contactsmember = temp_service->contacts; temp_contactsmember != NULL; temp_contactsmember = temp_contactsmember->next) {
			...
			if (temp_contact == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact '%s' specified in service '%s' for host '%s' is not defined anywhere!", temp_contactsmember->contact_name, temp_service->description, temp_service->host_name);
				errors++;
			}
		...
		for (temp_contactgroupsmember = temp_service->contact_groups; temp_contactgroupsmember != NULL; temp_contactgroupsmember = temp_contactgroupsmember->next) {
			...
			if (temp_contactgroup == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact group '%s' specified in service '%s' for host '%s' is not defined anywhere!", temp_contactgroupsmember->group_name, temp_service->description, temp_service->host_name);
				errors++;
			}
		...
		if (temp_service->contacts == NULL && temp_service->contact_groups == NULL) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Service '%s' on host '%s' has no default contacts or contactgroups defined!", temp_service->description, temp_service->host_name);
			warnings++;
		}

		/* verify service check timeperiod */
		if (temp_service->check_period == NULL) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Service '%s' on host '%s' has no check time period defined!", temp_service->description, temp_service->host_name);
			warnings++;
		} else {
			temp_timeperiod = find_timeperiod(temp_service->check_period);
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Check period '%s' specified for service '%s' on host '%s' is not defined anywhere!", temp_service->check_period, temp_service->description, temp_service->host_name);
				errors++;
			}
		...
		if (temp_service->notification_period == NULL) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Service '%s' on host '%s' has no notification time period defined!", temp_service->description, temp_service->host_name);
			warnings++;
		}
			...
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Notification period '%s' specified for service '%s' on host '%s' is not defined anywhere!", temp_service->notification_period, temp_service->description, temp_service->host_name);
				errors++;
			}
		...
		if (temp_service->notification_interval < temp_service->check_interval && temp_service->notification_interval != 0) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Service '%s' on host '%s'  has a notification interval less than its check interval!  Notifications are only re-sent after checks are made, so the effective notification interval will be that of the check interval.", temp_service->description, temp_service->host_name);
			warnings++;
		}
		...
		if (use_precached_objects == FALSE) {
			if (contains_illegal_object_chars(temp_service->description) == TRUE) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The description string for service '%s' on host '%s' contains one or more illegal characters.", temp_service->description, temp_service->host_name);
				errors++;
			}
		}
	}
	...
	if (get_host_count() == 0) {
		logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: 没有定义主机!");
		warnings++;
	}
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if(temp_host->total_services == 0 && verify_config >= 2) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: 没有与主机'%s'相关的服务!", temp_host->name);
			warnings++;
		}
		...
		if (temp_host->event_handler != NULL) {
			...
			temp_command = find_command(temp_command_name);
			if (temp_command == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Event handler command '%s' specified for host '%s' not defined anywhere", temp_command_name, temp_host->name);
				errors++;
			}
		...
		if (temp_host->host_check_command != NULL) {
			...
			temp_command = find_command(temp_command_name);
			if (temp_command == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Host check command '%s' specified for host '%s' is not defined anywhere!", temp_command_name, temp_host->name);
				errors++;
			}
		...
		if (temp_host->check_period != NULL) {
			...
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Check period '%s' specified for host '%s' is not defined anywhere!", temp_host->check_period, temp_host->name);
				errors++;
			}
		...
		for (temp_contactsmember = temp_host->contacts; temp_contactsmember != NULL; temp_contactsmember = temp_contactsmember->next) {
			...
			if (temp_contact == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact '%s' specified in host '%s' is not defined anywhere!", temp_contactsmember->contact_name, temp_host->name);
				errors++;
			}
		...
		for (temp_contactgroupsmember = temp_host->contact_groups; temp_contactgroupsmember != NULL; temp_contactgroupsmember = temp_contactgroupsmember->next) {
			...
			if (temp_contactgroup == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact group '%s' specified in host '%s' is not defined anywhere!", temp_contactgroupsmember->group_name, temp_host->name);
				errors++;
			}
		...
		if (temp_host->contacts == NULL && temp_host->contact_groups == NULL) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Host '%s' has no default contacts or contactgroups defined!", temp_host->name);
			warnings++;
		}
		...
		if (temp_host->notification_period != NULL) {
			...
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Notification period '%s' specified for host '%s' is not defined anywhere!", temp_host->notification_period, temp_host->name);
				errors++;
			}
		...
		for (temp_hostsmember = temp_host->parent_hosts; temp_hostsmember != NULL; temp_hostsmember = temp_hostsmember->next) {
			...
			if ((temp_host2 = find_host(temp_hostsmember->host_name)) == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: '%s' is not a valid parent for host '%s'!", temp_hostsmember->host_name, temp_host->name);
				errors++;
			}
		...
		if (temp_host->notify_on_recovery == TRUE && temp_host->notify_on_down == FALSE && temp_host->notify_on_unreachable == FALSE) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Recovery notification option in host '%s' definition doesn't make any sense - specify down and/or unreachable options as well", temp_host->name);
			warnings++;
		}
		...
		if (use_precached_objects == FALSE) {
			if (contains_illegal_object_chars(temp_host->name) == TRUE) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The name of host '%s' contains one or more illegal characters.", temp_host->name);
				errors++;
			}
		}
	}
	...
	for (temp_hostgroup = hostgroup_list, total_objects = 0; temp_hostgroup != NULL; temp_hostgroup = temp_hostgroup->next, total_objects++) {
		...
		for (temp_hostsmember = temp_hostgroup->members; temp_hostsmember != NULL; temp_hostsmember = temp_hostsmember->next) {
			...
			if (temp_host == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Host '%s' specified in host group '%s' is not defined anywhere!", temp_hostsmember->host_name, temp_hostgroup->group_name);
				errors++;
			}
		...
		if (use_precached_objects == FALSE) {
			if (contains_illegal_object_chars(temp_hostgroup->group_name) == TRUE) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The name of hostgroup '%s' contains one or more illegal characters.", temp_hostgroup->group_name);
				errors++;
			}
		}
	}
	...
	for (temp_servicegroup = servicegroup_list, total_objects = 0; temp_servicegroup != NULL; temp_servicegroup = temp_servicegroup->next, total_objects++) {
		...
		for (temp_servicesmember = temp_servicegroup->members; temp_servicesmember != NULL; temp_servicesmember = temp_servicesmember->next) {
			...
			if (temp_service == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Service '%s' on host '%s' specified in service group '%s' is not defined anywhere!", temp_servicesmember->service_description, temp_servicesmember->host_name, temp_servicegroup->group_name);
				errors++;
			}
		...
		if (use_precached_objects == FALSE) {
			if (contains_illegal_object_chars(temp_servicegroup->group_name) == TRUE) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The name of servicegroup '%s' contains one or more illegal characters.", temp_servicegroup->group_name);
				errors++;
			}
		}
	}
	...
	if (contact_list == NULL) {
		logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: 没有定义联系人!");
		warnings++;
	}
	for (temp_contact = contact_list, total_objects = 0; temp_contact != NULL; temp_contact = temp_contact->next, total_objects++) {
		...
		if (temp_contact->service_notification_commands == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact '%s' has no service notification commands defined!", temp_contact->name);
			errors++;
				...
				temp_command = find_command(temp_command_name);
				if (temp_command == NULL) {
					logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Service notification command '%s' specified for contact '%s' is not defined anywhere!", temp_command_name, temp_contact->name);
					errors++;
				}
		...
		if (temp_contact->host_notification_commands == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact '%s' has no host notification commands defined!", temp_contact->name);
			errors++;
				...
				temp_command = find_command(temp_command_name);
				if (temp_command == NULL) {
					logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Host notification command '%s' specified for contact '%s' is not defined anywhere!", temp_command_name, temp_contact->name);
					errors++;
				}
		...
		if (temp_contact->service_notification_period == NULL) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Contact '%s' has no service notification time period defined!", temp_contact->name);
			warnings++;
		}
			...
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Service notification period '%s' specified for contact '%s' is not defined anywhere!", temp_contact->service_notification_period, temp_contact->name);
				errors++;
			}
		...
		if (temp_contact->host_notification_period == NULL) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Contact '%s' has no host notification time period defined!", temp_contact->name);
			warnings++;
		}
			...
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Host notification period '%s' specified for contact '%s' is not defined anywhere!", temp_contact->host_notification_period, temp_contact->name);
				errors++;
			}
		...
		if (temp_contact->notify_on_host_recovery == TRUE && temp_contact->notify_on_host_down == FALSE && temp_contact->notify_on_host_unreachable == FALSE) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Host recovery notification option for contact '%s' doesn't make any sense - specify down and/or unreachable options as well", temp_contact->name);
			warnings++;
		}
		...
		if (temp_contact->notify_on_service_recovery == TRUE && temp_contact->notify_on_service_critical == FALSE && temp_contact->notify_on_service_warning == FALSE) {
			logit(NSLOG_VERIFICATION_WARNING, TRUE, "警报: Service recovery notification option for contact '%s' doesn't make any sense - specify critical and/or warning options as well", temp_contact->name);
			warnings++;
		}
		...
		if (use_precached_objects == FALSE) {
			if (contains_illegal_object_chars(temp_contact->name) == TRUE) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The name of contact '%s' contains one or more illegal characters.", temp_contact->name);
				errors++;
			}
		}
	}
	...
	for (temp_contactgroup = contactgroup_list, total_objects = 0; temp_contactgroup != NULL; temp_contactgroup = temp_contactgroup->next, total_objects++) {
		...
		for (temp_contactsmember = temp_contactgroup->members; temp_contactsmember != NULL; temp_contactsmember = temp_contactsmember->next) {
			...
			temp_contact = find_contact(temp_contactsmember->contact_name);
			if (temp_contact == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact '%s' specified in contact group '%s' is not defined anywhere!", temp_contactsmember->contact_name, temp_contactgroup->group_name);
				errors++;
			}
		...
		if (use_precached_objects == FALSE) {
			if (contains_illegal_object_chars(temp_contactgroup->group_name) == TRUE) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The name of contact group '%s' contains one or more illegal characters.", temp_contactgroup->group_name);
				errors++;
			}
		}
	}
	...
	for (temp_se = serviceescalation_list, total_objects = 0; temp_se != NULL; temp_se = temp_se->next, total_objects++) {
		...
		if (temp_service == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Service '%s' on host '%s' specified in service escalation is not defined anywhere!", temp_se->description, temp_se->host_name);
			errors++;
		}
		...
		if (temp_se->escalation_period != NULL) {
			...
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Escalation period '%s' specified in service escalation for service '%s' on host '%s' is not defined anywhere!", temp_se->escalation_period, temp_se->description, temp_se->host_name);
				errors++;
			}
		...
		for (temp_contactsmember = temp_se->contacts; temp_contactsmember != NULL; temp_contactsmember = temp_contactsmember->next) {
			...
			temp_contact = find_contact(temp_contactsmember->contact_name);
			if (temp_contact == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact '%s' specified in service escalation for service '%s' on host '%s' is not defined anywhere!", temp_contactsmember->contact_name, temp_se->description, temp_se->host_name);
				errors++;
			}
		...
		for (temp_contactgroupsmember = temp_se->contact_groups; temp_contactgroupsmember != NULL; temp_contactgroupsmember = temp_contactgroupsmember->next) {
			...
			if (temp_contactgroup == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact group '%s' specified in service escalation for service '%s' on host '%s' is not defined anywhere!", temp_contactgroupsmember->group_name, temp_se->description, temp_se->host_name);
				errors++;
			}
		...
		for (temp_escalation_condition = temp_se->condition; temp_escalation_condition != NULL; temp_escalation_condition = temp_escalation_condition->next) {
			...
			if (temp_host == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Host '%s' specified in service escalation condition is not defined anywhere!", temp_escalation_condition->host_name);
				errors++;
			}
				...
				if (temp_service == NULL) {
					logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Service '%s' on host '%s' specified in service escalation is not defined anywhere!", temp_escalation_condition->service_description, temp_escalation_condition->host_name);
					errors++;
				}
			}
		}
	...
	for (temp_sd = servicedependency_list, total_objects = 0; temp_sd != NULL; temp_sd = temp_sd->next, total_objects++) {
		...
		if (temp_service == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Dependent service '%s' on host '%s' specified in service dependency for service '%s' on host '%s' is not defined anywhere!", temp_sd->dependent_service_description, temp_sd->dependent_host_name, temp_sd->service_description, temp_sd->host_name);
			errors++;
		}
		...
		if (temp_service2 == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Service '%s' on host '%s' specified in service dependency for service '%s' on host '%s' is not defined anywhere!", temp_sd->service_description, temp_sd->host_name, temp_sd->dependent_service_description, temp_sd->dependent_host_name);
			errors++;
		}
		...
		if (temp_service == temp_service2) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Service dependency definition for service '%s' on host '%s' is circular (it depends on itself)!", temp_sd->dependent_service_description, temp_sd->dependent_host_name);
			errors++;
		}
		...
		if (temp_sd->dependency_period != NULL) {
			...
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Dependency period '%s' specified in service dependency for service '%s' on host '%s' is not defined anywhere!", temp_sd->dependency_period, temp_sd->dependent_service_description, temp_sd->dependent_host_name);
				errors++;
			}
	...
	for (temp_he = hostescalation_list, total_objects = 0; temp_he != NULL; temp_he = temp_he->next, total_objects++) {
		...
		if (temp_host == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Host '%s' specified in host escalation is not defined anywhere!", temp_he->host_name);
			errors++;
		}
		...
		if (temp_he->escalation_period != NULL) {
			...
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Escalation period '%s' specified in host escalation for host '%s' is not defined anywhere!", temp_he->escalation_period, temp_he->host_name);
				errors++;
			}
		...
		for (temp_contactsmember = temp_he->contacts; temp_contactsmember != NULL; temp_contactsmember = temp_contactsmember->next) {
			...
			temp_contact = find_contact(temp_contactsmember->contact_name);
			if (temp_contact == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact '%s' specified in host escalation for host '%s' is not defined anywhere!", temp_contactsmember->contact_name, temp_he->host_name);
				errors++;
			}
		...
		for (temp_contactgroupsmember = temp_he->contact_groups; temp_contactgroupsmember != NULL; temp_contactgroupsmember = temp_contactgroupsmember->next) {
			...
			if (temp_contactgroup == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Contact group '%s' specified in host escalation for host '%s' is not defined anywhere!", temp_contactgroupsmember->group_name, temp_he->host_name);
				errors++;
			}
	...
	for (temp_hd = hostdependency_list, total_objects = 0; temp_hd != NULL; temp_hd = temp_hd->next, total_objects++) {
		...
		if (temp_host == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Dependent host specified in host dependency for host '%s' is not defined anywhere!", temp_hd->dependent_host_name);
			errors++;
		}
		...
		if (temp_host2 == NULL) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Host specified in host dependency for host '%s' is not defined anywhere!", temp_hd->dependent_host_name);
			errors++;
		}
		...
		if (temp_host == temp_host2) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Host dependency definition for host '%s' is circular (it depends on itself)!", temp_hd->dependent_host_name);
			errors++;
		}
		...
		if (temp_hd->dependency_period != NULL) {
			...
			if (temp_timeperiod == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Dependency period '%s' specified in host dependency for host '%s' is not defined anywhere!", temp_hd->dependency_period, temp_hd->dependent_host_name);
				errors++;
			}
	...
	for (temp_command = command_list, total_objects = 0; temp_command != NULL; temp_command = temp_command->next, total_objects++) {
		...
		if (use_precached_objects == FALSE) {
			if (contains_illegal_object_chars(temp_command->name) == TRUE) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The name of command '%s' contains one or more illegal characters.", temp_command->name);
				errors++;
			}
		}
	}
	...
	for (temp_timeperiod = timeperiod_list, total_objects = 0; temp_timeperiod != NULL; temp_timeperiod = temp_timeperiod->next, total_objects++) {
		...
		if (use_precached_objects == FALSE) {
			if (contains_illegal_object_chars(temp_timeperiod->name) == TRUE) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The name of time period '%s' contains one or more illegal characters.", temp_timeperiod->name);
				errors++;
			}
		}
		...
		for (temp_timeperiodexclusion = temp_timeperiod->exclusions; temp_timeperiodexclusion != NULL; temp_timeperiodexclusion = temp_timeperiodexclusion->next) {
			...
			if (temp_timeperiod2 == NULL) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: Excluded time period '%s' specified in timeperiod '%s' is not defined anywhere!", temp_timeperiodexclusion->timeperiod_name, temp_timeperiod->name);
				errors++;
			}
	...
	for (temp_module = module_list, total_objects = 0; temp_module != NULL; temp_module = temp_module->next, total_objects++) {
		...
		if (use_precached_objects == FALSE) {
			if (contains_illegal_object_chars(temp_module->name) == TRUE) {
				logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The name of module '%s' contains one or more illegal characters.", temp_module->name);
				errors++;
			}
		}
	}
...
int pre_flight_circular_check(int *w, int *e) {
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		if (dfs_get_status(temp_host) == DFS_LOOPY)
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: The host '%s' is part of a circular parent/child chain!", temp_host->name);
		/* clean DFS status */
		dfs_set_status(temp_host, DFS_UNCHECKED);
	}
	...
	for (temp_sd = servicedependency_list; temp_sd != NULL; temp_sd = temp_sd->next) {
		...
		if (found == TRUE) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: A circular execution dependency (which could result in a deadlock) exists for service '%s' on host '%s'!", temp_sd->service_description, temp_sd->host_name);
			errors++;
		}
	}
	...
	for (temp_sd = servicedependency_list; temp_sd != NULL; temp_sd = temp_sd->next) {
		...
		if (found == TRUE) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: A circular notification dependency (which could result in a deadlock) exists for service '%s' on host '%s'!", temp_sd->service_description, temp_sd->host_name);
			errors++;
		}
	}
	...
	for (temp_hd = hostdependency_list; temp_hd != NULL; temp_hd = temp_hd->next) {
		...
		if (found == TRUE) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: 主机' %s' 存在一个执行依赖循环(这可能会导致一个死锁)!", temp_hd->host_name);
			errors++;
		}
	}
	...
	for (temp_hd = hostdependency_list; temp_hd != NULL; temp_hd = temp_hd->next) {
		...
		if (found == TRUE) {
			logit(NSLOG_VERIFICATION_ERROR, TRUE, "错误: 主机' %s' 存在一个通知依赖循环(这可能会导致一个死锁)!", temp_hd->host_name);
			errors++;
		}
	}
