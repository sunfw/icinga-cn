...
static const char *state_type_name(int state_type) {
        return state_type == HARD_STATE ? "硬件状态" : "软件状态";
}
...
int log_service_event(service *svc) {
	...
	if (log_long_plugin_output == TRUE && svc->long_plugin_output != NULL) {
		dummy = asprintf(&temp_buffer, "服务警告: %s;%s;%s;%s;%d;%s\\n%s\n",
...
int log_host_event(host *hst) {
	...
	if (log_long_plugin_output == TRUE && hst->long_plugin_output != NULL) {
		dummy = asprintf(&temp_buffer, "主机警告: %s;%s;%s;%d;%s\\n%s\n",
				hst->name,
...
int log_host_states(int type, time_t *timestamp) {
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		dummy = asprintf(&temp_buffer, "%s 主机状态: %s;%s;%s;%d;%s\n",
				(type == INITIAL_STATES) ? "初始" : "当前",
				temp_host->name,
...
int log_service_states(int type, time_t *timestamp) {
	...
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		dummy = asprintf(&temp_buffer, "%s 服务状态: %s;%s;%s;%s;%d;%s\n",
				(type == INITIAL_STATES) ? "初始" : "当前",
...
int rotate_log_file(time_t rotation_time) {
	...
	dummy = asprintf(&temp_buffer, "日志回滚: %s\n", method_string);
...
int write_log_file_info(time_t *timestamp) {
	...
	/* write log version */
	dummy = asprintf(&temp_buffer, "日志版本: %s\n", LOG_VERSION_2);
