...
int service_notification(service *svc, int type, char *not_author, char *not_data, int options) {
	...
	log_debug_info(DEBUGL_NOTIFICATIONS, 0, "** 服务通知尝试 ** 主机: '%s', 服务: '%s', 类型: %d, 选项: %d, 当前状态: %d, 最近通知: %s", svc->host_name, svc->description, type, options, svc->current_state, ctime(&svc->last_notification));

	/* if we couldn't find the host, return an error */
	if ((temp_host = svc->host_ptr) == NULL) {
		log_debug_info(DEBUGL_NOTIFICATIONS, 0, "找不到与此相关联的主机服务, 所以我们不会发送通知!\n");
		return ERROR;
	}

	/* check the viability of sending out a service notification */
	if (check_service_notification_viability(svc, type, options) == ERROR) {
		log_debug_info(DEBUGL_NOTIFICATIONS, 0, "通知可行性测试失败.  通知未发送出去.\n");
		return OK;
	}

	log_debug_info(DEBUGL_NOTIFICATIONS, 0, "通知可行性测试成功.\n");
	...
	*/
	...
	if (notification_list != NULL) {
		...
		if (type == NOTIFICATION_ACKNOWLEDGEMENT)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("确认");
		else if (type == NOTIFICATION_FLAPPINGSTART)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("抖动开始");
		else if (type == NOTIFICATION_FLAPPINGSTOP)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("抖动结束");
		else if (type == NOTIFICATION_FLAPPINGDISABLED)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("禁用抖动");
		else if (type == NOTIFICATION_DOWNTIMESTART)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("宕机开始");
		else if (type == NOTIFICATION_DOWNTIMEEND)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("宕机结束");
		else if (type == NOTIFICATION_DOWNTIMECANCELLED)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("取消宕机");
		else if (type == NOTIFICATION_CUSTOM)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("自定义");
		else if (type == NOTIFICATION_STALKING)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("跟踪");
		else if (svc->current_state == STATE_OK)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("恢复");
		else
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("故障");
		...
		if (type == NOTIFICATION_NORMAL) {
			...
			if (contacts_notified > 0) {
				...
				log_debug_info(DEBUGL_NOTIFICATIONS, 0, "已通知%d联系人.  下一个可能的通知时间: %s", contacts_notified, ctime(&svc->next_notification));
			...
			/* we didn't end up notifying anyone */
				...
				log_debug_info(DEBUGL_NOTIFICATIONS, 0, "没有通知联系人.  下次可能通知时间: %s", ctime(&svc->next_notification));
			}

		}

		log_debug_info(DEBUGL_NOTIFICATIONS, 0, "已通知%d联系人.\n", contacts_notified);
	}
	...
	/* there were no contacts, so no notification really occurred... */
		...
		log_debug_info(DEBUGL_NOTIFICATIONS, 0, "通知目标没有找到联系人.  无法发送通知.\n");
	}
...
int check_service_notification_viability(service *svc, int type, int options) {
	...
	if (enable_notifications == FALSE) {
		log_debug_info(DEBUGL_NOTIFICATIONS, 1, "已禁用通知, 因此服务通知将不会被发送出去.\n");
		return ERROR;
	}
	...
	/* if we couldn't find the host, return an error */
	if (temp_host == NULL) {
		log_debug_info(DEBUGL_NOTIFICATIONS, 1, "找不到与此服务相关联的主机, 因此我们不会发送通知.\n");
		return ERROR;
	}
	...
	if (check_time_against_period(current_time, temp_period) == ERROR) {
		...
		log_debug_info(DEBUGL_NOTIFICATIONS, 1, "该服务无法在这个时候发出通知.\n");
		...
		if (type == NOTIFICATION_NORMAL) {
			...
			log_debug_info(DEBUGL_NOTIFICATIONS, 1, "下一次可能通知时间: %s\n", ctime(&svc->next_notification));
		}
	...
	if (type == NOTIFICATION_CUSTOM) {
		if (svc->scheduled_downtime_depth > 0 || temp_host->scheduled_downtime_depth > 0) {
			log_debug_info(DEBUGL_NOTIFICATIONS, 1, "我们不会在安排宕机期间发送自定义通知.\n");
			return ERROR;
		}
		return OK;
	}
	...
	if (type == NOTIFICATION_ACKNOWLEDGEMENT) {
		...
		if (svc->current_state == STATE_OK) {
			log_debug_info(DEBUGL_NOTIFICATIONS, 1, "目前服务正常, 因此我们不会发送确认.\n");
			return ERROR;
		}
	...
	if ((current_time < svc->next_notification)) {
		if (svc->is_volatile == FALSE || svc->is_volatile == VOLATILE_WITH_RENOTIFICATION_INTERVAL) {
			...
			log_debug_info(DEBUGL_NOTIFICATIONS, 1, "下一个有效的通知时间: %s", ctime(&svc->next_notification));
			return ERROR;
		}
	}
...
int check_contact_service_notification_viability(contact *cntct, service *svc, int type, int options) {
	...
	if (svc->current_state == STATE_UNKNOWN && cntct->notify_on_service_unknown == FALSE) {
		log_debug_info(DEBUGL_NOTIFICATIONS, 2, "我们不应该通知该联系人关于未知的服务状态.\n");
		return ERROR;
	}

	if (svc->current_state == STATE_WARNING && cntct->notify_on_service_warning == FALSE) {
		log_debug_info(DEBUGL_NOTIFICATIONS, 2, "我们不应该通知该联系人关于警报的服务状态.\n");
		return ERROR;
	}

	if (svc->current_state == STATE_CRITICAL && cntct->notify_on_service_critical == FALSE) {
		log_debug_info(DEBUGL_NOTIFICATIONS, 2, "我们不应该通知该联系人关于严重的服务状态.\n");
		return ERROR;
	}
	...
	if (svc->current_state == STATE_OK) {
		...
		if (cntct->notify_on_service_recovery == FALSE) {
			log_debug_info(DEBUGL_NOTIFICATIONS, 2, "我们不应该通知该联系人关于恢复的服务状态.\n");
			return ERROR;
		}
		...
		if (!((svc->notified_on_unknown == TRUE && cntct->notify_on_service_unknown == TRUE) || (svc->notified_on_warning == TRUE && cntct->notify_on_service_warning == TRUE) || (svc->notified_on_critical == TRUE && cntct->notify_on_service_critical == TRUE))) {
			log_debug_info(DEBUGL_NOTIFICATIONS, 2, "关于该恢复我们不应该通知.\n");
			return ERROR;
		}
	}

	log_debug_info(DEBUGL_NOTIFICATIONS, 2, "** 联系人'%s'已通过服务通知可行性.\n", cntct->name);
...
int notify_contact_of_service(icinga_macros *mac, contact *cntct, service *svc, int type, char *not_author, char *not_data, int options, int escalated) {
	...
	log_debug_info(DEBUGL_NOTIFICATIONS, 2, "** 通知联系人 '%s'\n", cntct->name);
	...
	for (temp_commandsmember = cntct->service_notification_commands; temp_commandsmember != NULL; temp_commandsmember = temp_commandsmember->next) {
		...
		log_debug_info(DEBUGL_NOTIFICATIONS, 2, "原始通知命令: %s\n", raw_command);
		...
		log_debug_info(DEBUGL_NOTIFICATIONS, 2, "处理通知命令: %s\n", processed_command);

		/* log the notification to program log file */
		if (log_notifications == TRUE) {
			switch (type) {
			case NOTIFICATION_CUSTOM:
				dummy = asprintf(&temp_buffer, "服务通知: %s;%s;%s;自定义 ($SERVICESTATE$);%s;$SERVICEOUTPUT$;$NOTIFICATIONAUTHOR$;$NOTIFICATIONCOMMENT$\n", cntct->name, svc->host_name, svc->description, command_name_ptr);
				break;
			case NOTIFICATION_ACKNOWLEDGEMENT:
				dummy = asprintf(&temp_buffer, "服务通知: %s;%s;%s;确认 ($SERVICESTATE$);%s;$SERVICEOUTPUT$;$NOTIFICATIONAUTHOR$;$NOTIFICATIONCOMMENT$\n", cntct->name, svc->host_name, svc->description, command_name_ptr);
				break;
			case NOTIFICATION_FLAPPINGSTART:
				dummy = asprintf(&temp_buffer, "服务通知: %s;%s;%s;抖动开始 ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n", cntct->name, svc->host_name, svc->description, command_name_ptr);
				break;
			case NOTIFICATION_FLAPPINGSTOP:
				dummy = asprintf(&temp_buffer, "服务通知: %s;%s;%s;抖动停止 ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n", cntct->name, svc->host_name, svc->description, command_name_ptr);
				break;
			case NOTIFICATION_FLAPPINGDISABLED:
				dummy = asprintf(&temp_buffer, "服务通知: %s;%s;%s;禁用抖动 ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n", cntct->name, svc->host_name, svc->description, command_name_ptr);
				break;
			case NOTIFICATION_DOWNTIMESTART:
				dummy = asprintf(&temp_buffer, "服务通知: %s;%s;%s;宕机开始 ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n", cntct->name, svc->host_name, svc->description, command_name_ptr);
				break;
			case NOTIFICATION_DOWNTIMEEND:
				dummy = asprintf(&temp_buffer, "服务通知: %s;%s;%s;宕机结束 ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n", cntct->name, svc->host_name, svc->description, command_name_ptr);
				break;
			case NOTIFICATION_DOWNTIMECANCELLED:
				dummy = asprintf(&temp_buffer, "服务通知: %s;%s;%s;取消宕机 ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n", cntct->name, svc->host_name, svc->description, command_name_ptr);
				break;
			default:
				dummy = asprintf(&temp_buffer, "服务通知: %s;%s;%s;$SERVICESTATE$;%s;$SERVICEOUTPUT$\n", cntct->name, svc->host_name, svc->description, command_name_ptr);
				break;
			}
		...
		if (early_timeout == TRUE) {
			logit(NSLOG_SERVICE_NOTIFICATION | NSLOG_RUNTIME_WARNING, TRUE, "警报: 联系人 '%s' 服务通知命令 '%s' 超时%d秒\n", cntct->name, processed_command, notification_timeout);
		}
...
int check_escalation_condition(escalation_condition *cond) {
	...
	while (cnd != NULL) {
		...
		if (cnd->service_description != NULL && strcmp(cnd->service_description, "")
			...
			log_debug_info(DEBUGL_NOTIFICATIONS, 1, "检查服务升级条 %s.%s = %d.\n", svc->host_name, svc->description, result);
		}
		...
		/* host condition */
			...
			log_debug_info(DEBUGL_NOTIFICATIONS, 1, "检查主机升级条件 %s = %d.\n", hst->name, result);
		}
...
int host_notification(host *hst, int type, char *not_author, char *not_data, int options) {
	...
	log_debug_info(DEBUGL_NOTIFICATIONS, 0, "** 主机通知尝试 ** 主机: '%s', 类型: %d, 选项: %d, 当前状态: %d, 最近通知: %s", hst->name, type, options, hst->current_state, ctime(&hst->last_host_notification));
	...
	if (notification_list != NULL) {
		...
		if (type == NOTIFICATION_ACKNOWLEDGEMENT)
			...
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("抖动停止");
		else if (type == NOTIFICATION_FLAPPINGDISABLED)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("禁用抖动");
		else if (type == NOTIFICATION_DOWNTIMESTART)
			mac.x[MACRO_NOTIFICATIONTYPE] = (char *)strdup("宕机开始");
		...
		if (type == NOTIFICATION_NORMAL) {
			...
			if (contacts_notified > 0) {
				...
				log_debug_info(DEBUGL_NOTIFICATIONS, 0, "已通知联系人 %d .  下次可能通知时间: %s", contacts_notified, ctime(&hst->next_host_notification));
			}
			...
			/* we didn't end up notifying anyone */
				...
				log_debug_info(DEBUGL_NOTIFICATIONS, 0, "没有通知联系人.  下次可能通知时间: %s", ctime(&hst->next_host_notification));
			}
		}

		log_debug_info(DEBUGL_NOTIFICATIONS, 0, "已通知联系人 %d.\n", contacts_notified);
	}
...
int check_host_notification_viability(host *hst, int type, int options) {
	...
	/* forced notifications bust through everything */
	if (options & NOTIFICATION_OPTION_FORCED) {
		log_debug_info(DEBUGL_NOTIFICATIONS, 1, "这是被迫的主机通知, 因此我们将它发送出去.\n");
		return OK;
	}
	...
	/* are notifications enabled? */
	if (enable_notifications == FALSE) {
		log_debug_info(DEBUGL_NOTIFICATIONS, 1, "已禁用通知, 因此主机通知不会被发送出去.\n");
		return ERROR;
	}
	...
	if (type == NOTIFICATION_ACKNOWLEDGEMENT) {
		...
		if (hst->current_state == HOST_UP) {
			log_debug_info(DEBUGL_NOTIFICATIONS, 1, "目前主机在运行, 因此我们不会发送确认.\n");
			return ERROR;
		}
...
int notify_contact_of_host(icinga_macros *mac, contact *cntct, host *hst, int type, char *not_author, char *not_data, int options, int escalated) {
	...
	for (temp_commandsmember = cntct->host_notification_commands; temp_commandsmember != NULL; temp_commandsmember = temp_commandsmember->next) {
		...
		if (log_notifications == TRUE) {
			switch (type) {
			case NOTIFICATION_CUSTOM:
				dummy = asprintf(&temp_buffer, "主机通知: %s;%s;自定义 ($HOSTSTATE$);%s;$HOSTOUTPUT$;$NOTIFICATIONAUTHOR$;$NOTIFICATIONCOMMENT$\n", cntct->name, hst->name, command_name_ptr);
				break;
			case NOTIFICATION_ACKNOWLEDGEMENT:
				dummy = asprintf(&temp_buffer, "主机通知: %s;%s;确认 ($HOSTSTATE$);%s;$HOSTOUTPUT$;$NOTIFICATIONAUTHOR$;$NOTIFICATIONCOMMENT$\n", cntct->name, hst->name, command_name_ptr);
				break;
			case NOTIFICATION_FLAPPINGSTART:
				dummy = asprintf(&temp_buffer, "主机通知: %s;%s;抖动开始 ($HOSTSTATE$);%s;$HOSTOUTPUT$\n", cntct->name, hst->name, command_name_ptr);
				break;
			case NOTIFICATION_FLAPPINGSTOP:
				dummy = asprintf(&temp_buffer, "主机通知: %s;%s;抖动停止 ($HOSTSTATE$);%s;$HOSTOUTPUT$\n", cntct->name, hst->name, command_name_ptr);
				break;
			case NOTIFICATION_FLAPPINGDISABLED:
				dummy = asprintf(&temp_buffer, "主机通知: %s;%s;禁用抖动 ($HOSTSTATE$);%s;$HOSTOUTPUT$\n", cntct->name, hst->name, command_name_ptr);
				break;
			case NOTIFICATION_DOWNTIMESTART:
				dummy = asprintf(&temp_buffer, "主机通知: %s;%s;宕机开始 ($HOSTSTATE$);%s;$HOSTOUTPUT$\n", cntct->name, hst->name, command_name_ptr);
				break;
			case NOTIFICATION_DOWNTIMEEND:
				dummy = asprintf(&temp_buffer, "主机通知: %s;%s;宕机结束 ($HOSTSTATE$);%s;$HOSTOUTPUT$\n", cntct->name, hst->name, command_name_ptr);
				break;
			case NOTIFICATION_DOWNTIMECANCELLED:
				dummy = asprintf(&temp_buffer, "主机通知: %s;%s;取消宕机 ($HOSTSTATE$);%s;$HOSTOUTPUT$\n", cntct->name, hst->name, command_name_ptr);
				break;
			default:
				dummy = asprintf(&temp_buffer, "主机通知: %s;%s;$HOSTSTATE$;%s;$HOSTOUTPUT$\n", cntct->name, hst->name, command_name_ptr);
				break;
			}
...
time_t get_next_service_notification_time(service *svc, time_t offset) {
	...
	log_debug_info(DEBUGL_NOTIFICATIONS, 2, "默认间隔: %f\n", interval_to_use);
	...
	for (temp_se = serviceescalation_list; temp_se != NULL; temp_se = temp_se->next) {
		...
		log_debug_info(DEBUGL_NOTIFICATIONS, 2, "新间隔: %f\n", interval_to_use);
	}
...
int add_notification(icinga_macros *mac, contact *cntct) {
	...
	log_debug_info(DEBUGL_NOTIFICATIONS, 2, "添加联系人 '%s' 到通知列表.\n", cntct->name);
