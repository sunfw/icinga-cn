...
void init_timing_loop(void) {
	...
	for (temp_service = service_list; temp_service != NULL; temp_service = temp_service->next) {
		...
		if (schedule_check == TRUE) {
			...
			log_debug_info(DEBUGL_EVENTS, 1, "不应该安排主机'%s'上的服务'%s' .\n", temp_service->host_name, temp_service->description);
		}
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		if (schedule_check == TRUE) {
			...
			log_debug_info(DEBUGL_EVENTS, 1, "不应该安排主机'%s'.\n", temp_host->name);
		}
	...
	log_debug_info(DEBUGL_EVENTS, 2, "确定服务的安排参数...");
	...
	default:
		...
		log_debug_info(DEBUGL_EVENTS, 1, "安排的总计服务检查:  %d\n", scheduling_info.total_scheduled_services);
		log_debug_info(DEBUGL_EVENTS, 1, "平均服务检查间隔时间:  %0.2f sec\n", scheduling_info.average_service_check_interval);
		log_debug_info(DEBUGL_EVENTS, 1, "服务间检查间隔:       %0.2f sec\n", scheduling_info.service_inter_check_delay);
	}
	...
	default:
		...
		log_debug_info(DEBUGL_EVENTS, 1, "安排的总计服务检查: %d\n", scheduling_info.total_scheduled_services);
		log_debug_info(DEBUGL_EVENTS, 1, "总计主机:                    %d\n", scheduling_info.total_hosts);
		log_debug_info(DEBUGL_EVENTS, 1, "服务交错因素:      %d\n", scheduling_info.service_interleave_factor);
	}
	...
	log_debug_info(DEBUGL_EVENTS, 1, "安排的总计服务: %d\n", scheduling_info.total_scheduled_services);
	log_debug_info(DEBUGL_EVENTS, 1, "服务交错因素: %d\n", scheduling_info.service_interleave_factor);
	log_debug_info(DEBUGL_EVENTS, 1, "总计服务交错块: %d\n", total_interleave_blocks);
	log_debug_info(DEBUGL_EVENTS, 1, "服务间检查间隔: %2.1f\n", scheduling_info.service_inter_check_delay);
	...
	log_debug_info(DEBUGL_EVENTS, 2, "安排服务检查...");
	...
	for (temp_service = service_list; temp_service != NULL && scheduling_info.service_interleave_factor > 0;) {
		...
		log_debug_info(DEBUGL_EVENTS, 2, "目前交错块: %d\n", current_interleave_block);
		...
		for (interleave_block_index = 0; interleave_block_index < scheduling_info.service_interleave_factor && temp_service != NULL; temp_service = temp_service->next) {
			...
			log_debug_info(DEBUGL_EVENTS, 2, "多个因素: %d\n", mult_factor);
			...
			log_debug_info(DEBUGL_EVENTS, 2, "首选的检查时间: %lu --> %s", (unsigned long)temp_service->next_check, ctime(&temp_service->next_check));
			...
			log_debug_info(DEBUGL_EVENTS, 2, "实际检查时间: %lu --> %s", (unsigned long)temp_service->next_check, ctime(&temp_service->next_check));
	...
	log_debug_info(DEBUGL_EVENTS, 2, "确定主机的安排参数...");
	...
	default:
		...
		log_debug_info(DEBUGL_EVENTS, 2, "安排总计主机检查:  %d\n", scheduling_info.total_scheduled_hosts);
		log_debug_info(DEBUGL_EVENTS, 2, "主机检查间隔总计:    %lu\n", scheduling_info.host_check_interval_total);
		log_debug_info(DEBUGL_EVENTS, 2, "均的主机检查间隔:  %0.2f sec\n", scheduling_info.average_host_check_interval);
		log_debug_info(DEBUGL_EVENTS, 2, "主机间检查延迟:       %0.2f sec\n", scheduling_info.host_inter_check_delay);
	}
	...
	for (temp_host = host_list; temp_host != NULL; temp_host = temp_host->next) {
		...
		log_debug_info(DEBUGL_EVENTS, 2, "首选的检查时间: %lu --> %s", (unsigned long)temp_host->next_check, ctime(&temp_host->next_check));
		...
		log_debug_info(DEBUGL_EVENTS, 2, "实际检查时间: %lu --> %s", (unsigned long)temp_host->next_check, ctime(&temp_host->next_check));
...
void display_scheduling_info(void) {
	...
	printf("预计在下面列出主机和服务检查安排信息\n");
	printf("该信息假定您将开始使用您的当前配置文件运行%s.\n\n", PROGRAM_NAME);
	...
	if (service_inter_check_delay_method == ICD_NONE)
		...
		printf("SMART\n");
		printf("平均服务检查间隔:     %.2f sec\n", scheduling_info.average_service_check_interval);
	} else
		printf("USER-SUPPLIED 值\n");
	printf("检查间的延迟:                  %.2f 秒\n", scheduling_info.service_inter_check_delay);

	printf("交错因素方法:           %s\n", (service_interleave_factor_method == ILF_USER) ? "USER-SUPPLIED 值" : "SMART");
	...
	printf("安排首个检查:              %s", ctime(&scheduling_info.first_service_check));
	printf("安排最后检查:               %s", ctime(&scheduling_info.last_service_check));
	printf("\n\n");
	...
	if ((int)max_reaper_interval < check_reaper_interval) {
		printf("* Value for 'check_result_reaper_frequency' should be <= %d 秒\n", (int)max_reaper_interval);
		suggestions++;
	}
	if (check_reaper_interval < 2) {
		printf("* Value for 'check_result_reaper_frequency' should be >= 2 秒\n");
		suggestions++;
	}
...
int delete_scheduled_event(int event_type, int high_priority, time_t run_time, int recurring, unsigned long event_interval, void *timing_func, int compensate_for_time_change, void *event_data, void *event_args, int event_options) {
	...
	for (temp_event = *event_list; temp_event != NULL; temp_event = temp_event->next) {
		...
		if (temp_event->event_type == event_type && temp_event->event_options == event_options && temp_event->event_data == event_data) {
			...
			log_debug_info(DEBUGL_EVENTS, 1, "移除事件类型 %d @ %s", event_type, ctime(&run_time));
...
int event_execution_loop(void) {
	...
	while (1) {
		...
		if (event_list_high == NULL && event_list_low == NULL) {
			log_debug_info(DEBUGL_EVENTS, 0, "没有任何需要处理的事件!退出...\n");
			break;
		}
		...
		log_debug_info(DEBUGL_EVENTS, 1, "** 事件循环校验\n");
		if (event_list_high != NULL)
			log_debug_info(DEBUGL_EVENTS, 1, "下一个高优先级事件时间: %s", ctime(&event_list_high->run_time));
		else
			log_debug_info(DEBUGL_EVENTS, 1, "没有安排高优先级事件...\n");
		if (event_list_low != NULL)
			log_debug_info(DEBUGL_EVENTS, 1, "下一个低优先级事件时间:  %s", ctime(&event_list_low->run_time));
		else
			log_debug_info(DEBUGL_EVENTS, 1, "没有安排低优先级事件...\n");
		log_debug_info(DEBUGL_EVENTS, 1, "目前/最大服务检查: %d/%d\n", currently_running_service_checks, max_parallel_service_checks);
		...
		/* handle low priority events */
			...
			if (event_list_low->event_type == EVENT_SERVICE_CHECK) {
				...
				if (max_parallel_service_checks != 0 && (currently_running_service_checks >= max_parallel_service_checks)) {
					...
					log_debug_info(DEBUGL_EVENTS | DEBUGL_CHECKS, 0, "**警报** 已达到最大并发服务检查 (%d)!  微移%s:%s 的 %d 秒...\n", max_parallel_service_checks, temp_service->host_name, temp_service->description, nudge_seconds);

					logit(NSLOG_RUNTIME_WARNING, TRUE, "\t已达到最大并发服务检查 (%d).  微移%s:%s 的%d 秒...\n", max_parallel_service_checks, temp_service->host_name, temp_service->description, nudge_seconds);
					run_event = FALSE;
				}
				...
				if (run_event == FALSE) {
					...
					log_debug_info(DEBUGL_EVENTS | DEBUGL_CHECKS, 1, "跳过事件, 从列表移除服务 '%s'.\n", temp_service->description);
			...
			if (run_event == TRUE) {
				...
				log_debug_info(DEBUGL_EVENTS, 1, "运行事件...\n");
...
int handle_timed_event(timed_event *event) {
	...
	log_debug_info(DEBUGL_EVENTS, 0, "** 定时事件 ** 类型: %d, 运行时间: %s", event->event_type, ctime(&event->run_time));
	...
	case EVENT_HOST_CHECK:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 主机检查事件 ==> 主机: '%s', 选项: %d, 延迟: %f 秒\n", temp_host->name, event->event_options, latency);
	...
	case EVENT_COMMAND_CHECK:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 额外命令检查事件\n");
	...
	case EVENT_LOG_ROTATION:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 日志文件回滚事件\n");
	...
	case EVENT_PROGRAM_SHUTDOWN:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 应用程序关闭事件\n");
	...
	case EVENT_PROGRAM_RESTART:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 应用程序重启事件\n");
	...
	case EVENT_EXPIRE_DISABLED_NOTIFICATIONS:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 禁用逾期通知事件\n");
		...
		if (disable_notifications_expire_time > 0) {
			...
			enable_all_notifications();
		        ...
		        logit(NSLOG_INFO_MESSAGE, TRUE, "禁用通知逾期. 重新启用所有通知.\n");
		}
	...
	case EVENT_CHECK_REAPER:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 检查结果收割程式\n");
	...
	case EVENT_ORPHAN_CHECK:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 孤立的主机和服务检查事件\n");
	...
	case EVENT_RETENTION_SAVE:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 保留数据保存事件\n");
	...
	case EVENT_STATUS_SAVE:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 状态数据保存事件\n");
	...
	case EVENT_SCHEDULED_DOWNTIME:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 安排宕机事件\n");
	...
	case EVENT_SFRESHNESS_CHECK:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 服务结果更新检查事件\n");
	...
	case EVENT_HFRESHNESS_CHECK:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 主机结果更新检查事件\n");
	...
	case EVENT_EXPIRE_DOWNTIME:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 逾期宕机事件\n");
	...
	case EVENT_EXPIRE_ACKNOWLEDGEMENT:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 逾期确认事件\n");
		...
		if (event->event_options == SERVICE_ACKNOWLEDGEMENT) {
			...
			log_debug_info(DEBUGL_EVENTS, 0, "** 未知逾期确认事件: %d\n", event->event_options);
		}
	...
	case EVENT_RESCHEDULE_CHECKS:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 重新安排检查事件\n");
	...
	case EVENT_EXPIRE_COMMENT:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 逾期注释事件\n");
	...
	case EVENT_USER_FUNCTION:
		...
		log_debug_info(DEBUGL_EVENTS, 0, "** 用户函数事件\n");
