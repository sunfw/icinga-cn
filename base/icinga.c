...
#undef TEST_TIMEPERIODS
		...
		printf("=====\n");
		printf("当前:   %lu = %s", (unsigned long)now, ctime(&now));
		printf("首选: %lu = %s", (unsigned long)pref_time, ctime(&pref_time));
		printf("下一次:      %lu = %s", (unsigned long)valid_time, ctime(&valid_time));
		printf("=====\n");
#endif
	...
	/* else start to monitor things... */
		...
		do {
			...
			logit(NSLOG_PROCESS_INFO, TRUE, "%s %s 开始... (PID=%d)\n", PROGRAM_NAME, PROGRAM_VERSION, (int)getpid());

			/* log the local time - may be different than clock time due to timezone offset */
			now = time(NULL);
			tm = localtime_r(&now, &tm_s);
			snprintf(datestring,sizeof(datestring),"%s %04d年%02d月%02d日 %02d时:%02d分:%02d秒 %s",weekdays[tm->tm_wday],tm->tm_year+1900,tm->tm_mon+1,tm->tm_mday,tm->tm_hour,tm->tm_min,tm->tm_sec,tm->tm_zone); //
			logit(NSLOG_PROCESS_INFO, TRUE, "本地时间 %s", datestring);
			...
			if (neb_load_all_modules() != OK) {
                logit(NSLOG_CONFIG_ERROR, ERROR, "错误: NEB module loading failed. Aborting.\n");
			...
			if (daemon_mode == TRUE && sigrestart == FALSE) {
				...
				dummy = asprintf(&buffer, "完成守护进程... (新 PID=%d)\n", (int)getpid());
			...
			if (caught_signal == TRUE) {
				...
				if (sig_id == SIGHUP)
					dummy = asprintf(&buffer, "Caught SIGHUP, 重启...\n");
				else if (sig_id != SIGSEGV)
					dummy = asprintf(&buffer, "Caught SIG%s, 关闭...\n", sigs[sig_id]);
			...
			if (sigshutdown == TRUE) {
				...
				logit(NSLOG_PROCESS_INFO, TRUE, "成功关闭... (PID=%d)\n", (int)getpid());
			}
