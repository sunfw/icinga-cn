...
int process_external_commands_from_file(char *fname, int delete_file) {
	...
	if ((thefile = mmap_fopen(fname)) == NULL) {
		logit(NSLOG_INFO_MESSAGE, FALSE, "错误: 无法打开文件 '%s' 处理额外命令!", fname);
		return ERROR;
	}
...
int process_external_command1(char *cmd) {
	...
	log_debug_info(DEBUGL_EXTERNALCOMMANDS, 2, "Raw命令条目: %s\n", cmd);
	...
	/**** UNKNOWN COMMAND ****/
		...
		logit(NSLOG_EXTERNAL_COMMAND | NSLOG_RUNTIME_WARNING, TRUE, "警报: 无法识别的额外命令 -> %s;%s\n", command_id, args);
	...
	dummy = asprintf(&temp_buffer, "额外命令: %s;%s\n", command_id, args);
...
int process_external_command2(int cmd, time_t entry_time, char *args) {
	...
	log_debug_info(DEBUGL_EXTERNALCOMMANDS, 1, "额外命令类型: %d\n", cmd);
	log_debug_info(DEBUGL_EXTERNALCOMMANDS, 1, "命令输入时间: %lu\n", (unsigned long)entry_time);
	log_debug_info(DEBUGL_EXTERNALCOMMANDS, 1, "命令参数: %s\n", (args == NULL) ? "" : args);
...
int process_host_command(int cmd, time_t entry_time, char *args) {
	...
	if ((temp_host = find_host(host_name)) == NULL) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 额外命令无法找到提供的主机'%s'!\n", host_name);
		return ERROR;
	}
...
int process_hostgroup_command(int cmd, time_t entry_time, char *args) {
	...
	if ((temp_hostgroup = find_hostgroup(hostgroup_name)) == NULL) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 额外命令无法找到提供的主机组'%s'!\n", hostgroup_name);
		return ERROR;
	}
...
int process_service_command(int cmd, time_t entry_time, char *args) {
	...
	if ((temp_service = find_service(host_name, svc_description)) == NULL) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 额外命令无法找到提供的主机'%s'和服务'%s'!\n", host_name, svc_description);
		return ERROR;
	}
...
int process_servicegroup_command(int cmd, time_t entry_time, char *args) {
	...
	if ((temp_servicegroup = find_servicegroup(servicegroup_name)) == NULL) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 额外命令无法找到提供的服务组'%s'!\n", servicegroup_name);
		return ERROR;
	}
...
int process_contact_command(int cmd, time_t entry_time, char *args) {
	...
	if ((temp_contact = find_contact(contact_name)) == NULL) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 额外命令无法找到提供的联系人'%s'!\n", contact_name);
		return ERROR;
	}
...
int process_contactgroup_command(int cmd, time_t entry_time, char *args) {
	...
	if ((temp_contactgroup = find_contactgroup(contactgroup_name)) == NULL) {
		logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 额外命令无法找到提供的联系人组'%s'!\n", contactgroup_name);
		return ERROR;
	}
...
int process_passive_service_check(time_t check_time, char *host_name, char *svc_description, int return_code, char *output) {
	...
	if (real_host_name == NULL) {
		logit(NSLOG_RUNTIME_WARNING, TRUE, "警报:  Passive check result was received for service '%s' on host '%s', but the host could not be found!\n", svc_description, host_name);
		return ERROR;
	}

	/* make sure the service exists */
	if ((temp_service = find_service(real_host_name, svc_description)) == NULL) {
		logit(NSLOG_RUNTIME_WARNING, TRUE, "警报:  Passive check result was received for service '%s' on host '%s', but the service could not be found!\n", svc_description, host_name);
		return ERROR;
	}
...
int process_passive_host_check(time_t check_time, char *host_name, int return_code, char *output) {
	...
	/* we couldn't find the host */
	if (temp_host == NULL) {
		logit(NSLOG_RUNTIME_WARNING, TRUE, "警报:  Passive check result was received for host '%s', but the host could not be found!\n", host_name);
		return ERROR;
	}
...
int cmd_schedule_downtime(int cmd, time_t entry_time, char *args) {
	...
	if (cmd == CMD_SCHEDULE_HOSTGROUP_HOST_DOWNTIME || cmd == CMD_SCHEDULE_HOSTGROUP_SVC_DOWNTIME) {
		...
		if ((temp_hostgroup = find_hostgroup(hostgroup_name)) == NULL) {
			logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 无法找到主机组 '%s' provided in external command!\n", hostgroup_name);
			return ERROR;
		}
	}
		...
		if ((temp_servicegroup = find_servicegroup(servicegroup_name)) == NULL) {
			logit(NSLOG_RUNTIME_ERROR, TRUE, "错误: 无法找到服务组 '%s' provided in external command!\n", servicegroup_name);
			return ERROR;
		}
	}
